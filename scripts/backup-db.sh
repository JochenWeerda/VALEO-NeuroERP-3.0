***REMOVED***!/bin/bash
***REMOVED*** PostgreSQL Backup Script for VALEO-NeuroERP
***REMOVED*** Usage: ./scripts/backup-db.sh
...
***REMOVED*** Monthly backup (on 1st of month)
if [ "$(date +%d)" = "01" ]; then
  cp "${BACKUP_DIR}/daily/${BACKUP_FILE}" "${BACKUP_DIR}/monthly/${BACKUP_FILE}"
  echo "Monthly backup created: ${BACKUP_DIR}/monthly/${BACKUP_FILE}"
fi

***REMOVED*** Cleanup old backups
echo "Cleaning up old backups..."

***REMOVED*** Remove daily backups older than RETENTION_DAYS
find "${BACKUP_DIR}/daily" -name "*.sql.gz" -mtime +${RETENTION_DAYS} -delete

***REMOVED*** Remove monthly backups older than RETENTION_MONTHS
...
***REMOVED*** Upload to S3/Azure (optional)
if [ -n "${S3_BUCKET}" ]; then
