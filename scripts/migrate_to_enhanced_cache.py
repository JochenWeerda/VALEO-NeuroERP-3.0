...
***REMOVED*** Tag-Definitionen für verschiedene API-Module
API_TAGS = {
    "charges_api.py": {
        "module": "charges",
        "entity": "charge",
...
***REMOVED*** Standard-Tags für alle API-Module
...
def detect_endpoints(content: str) -> List[Dict[str, Any]]:
    ...
    ***REMOVED*** Regex-Muster für FastAPI-Endpunkte
    ...
    for match in re.finditer(endpoint_pattern, content, re.MULTILINE):
        ...
        ***REMOVED*** Überprüfen, ob ein Cache-Dekorator vorhanden ist
        start_pos = match.start()
        line_before = content[:start_pos].rfind("\n")
        has_cache = "@cache.cached" in content[line_before:start_pos]
        
        ***REMOVED*** Parameter aus dem Pfad extrahieren
...
def generate_tags_for_endpoint(endpoint: Dict[str, Any], api_tags: Dict[str, Any]) -> List[str]:
    ...
    ***REMOVED*** Modul-Tag hinzufügen
    tags.append(api_tags["module"])
    
    ***REMOVED*** Methode berücksichtigen
    if endpoint["method"] == "get":
        if not endpoint["path_params"]:
            ***REMOVED*** Listen-Endpunkt
            tags.append("list")
        else:
            ***REMOVED*** Detail-Endpunkt
            tags.append("details")
            
            ***REMOVED*** Entity-Tag mit ID hinzufügen
            for param in endpoint["path_params"]:
                if "id" in param.lower():
                    tags.append(f"{api_tags['entity']}:{{{param}}}")
    
    ***REMOVED*** Zusätzliche Tags basierend auf dem Pfad
    path_lower = endpoint["path"].lower()
    
    ***REMOVED*** Stats-Endpunkte
    if "stats" in path_lower or "statistics" in path_lower:
        tags.append("stats")
    
    ***REMOVED*** Spezifische Tags aus dem API-Modul
...
def migrate_file(file_path: str, dry_run: bool = False) -> bool:
    ...
    try:
        ***REMOVED*** Dateinamen extrahieren
        filename = os.path.basename(file_path)
        
        ***REMOVED*** Tags für dieses API-Modul
        ...
        ***REMOVED*** Sicherstellen, dass wir nur einmal migrieren
        if NEW_IMPORT in content:
            logger.warning(f"Datei wurde bereits migriert: {file_path}")
            return False
        
        ***REMOVED*** Änderungen verfolgen
        changes = []
        
        ***REMOVED*** Import ändern
        ...
        ***REMOVED*** Logging hinzufügen, falls nicht vorhanden
        ...
        ***REMOVED*** Logger-Definition hinzufügen, falls nicht vorhanden
        ...
        ***REMOVED*** Cache-Invalidierung für Mutations-Endpunkte hinzufügen
        for ep in endpoints:
            if ep["method"] in ["post", "put", "delete", "patch"] and not ep["has_cache"]:
                ***REMOVED*** Funktion finden
                func_pattern = f"async def {ep['function']}\\([^)]*\\):[^\\n]*\\n"
                func_match = re.search(func_pattern, new_content)
                
                if func_match:
                    ***REMOVED*** Nach dem return-Statement suchen
                    func_start = func_match.end()
                    func_end = None
                    
                    ***REMOVED*** Nächste Funktion oder Dateiende suchen
                    ...
                    ***REMOVED*** Return-Statement finden
                    ...
                    if return_match:
                        ...
                        ***REMOVED*** Cache-Invalidierungscode erstellen
                        invalidation_code = f"\n{indentation}***REMOVED*** Cache-Invalidierung\n"
                        
                        ***REMOVED*** ID-Parameter finden
        ...
        ***REMOVED*** Cache-Stats-Endpunkt hinzufügen, falls nicht vorhanden
        if not any(ep["path"].endswith("/cache-stats") for ep in endpoints):
            ***REMOVED*** Position für den neuen Endpunkt finden (nach dem letzten Endpunkt)
            last_endpoint_pos = 0
            for ep in endpoints:
                func_pattern = f"async def {ep['function']}\\([^)]*\\):[^\\n]*\\n"
                func_match = re.search(func_pattern, new_content)
            ...
            if last_endpoint_pos > 0:
                ***REMOVED*** Einrückung ermitteln
                ...
                if last_lines:
                    ...
                    indentation = "    "  ***REMOVED*** Standard-Einrückung
                
                ***REMOVED*** Neuen Endpunkt erstellen
        ...
        ***REMOVED*** Änderungen anzeigen
        for change in changes:
            logger.info(f"  - {change}")
        
        ***REMOVED*** Datei speichern, wenn nicht im Dry-Run-Modus
...
def main():
    ...
    if args.all:
        ***REMOVED*** Alle API-Dateien migrieren
        ...
        ***REMOVED*** Einzelne Datei migrieren
