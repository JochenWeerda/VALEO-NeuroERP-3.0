...
def analyze_filtered_26xxx():
    ***REMOVED*** Pfad zur vorgefilteren CSV
    ...
    print("ANALYSE DER VORGEFILTEREN PLZ 26XXX-DATEN")
    print("=" * 60)
    ...
    ***REMOVED*** Flächenprämien-Codes (basierend auf GAP-Systematik)
    area_premium_codes = {
        'I.1': 'Basis-Direktzahlung (Flaechenprämie)',
        'I.2': 'Umverteilungsz. fuer Nachhaltigkeit (Flaechenprämie)', 
        'I.3': 'Zahlung fuer Junglandwirte (Flaechenprämie)',
        'I.4': 'Gekoppelte Einkommenstuetzung (Flaechenprämie)', 
        'I.6': 'Kleinerzeugerregelung (Flaechenprämie)',
        'V.1': 'Agrarumwelt-Klimamassnahmen (meist flaechenbezogen)',
    }
    ...
    ***REMOVED*** Verschiedene Encodings versuchen
            ...
            with open(csv_path, 'r', encoding=encoding) as f:
                ...
                ***REMOVED*** Header-Mapping finden
                ...
                for header in headers:
                    ...
                    elif 'beguenstigten' in header.lower() and 'rechtstraegers' in header.lower():
                        name_header = header
                    elif 'code der massnahme' in header.lower():
                ...
                for row_num, row in enumerate(reader, 1):
                    ...
                    ***REMOVED*** NUR Flaechenprämien-Codes verarbeiten
                    if measure_code in area_premium_codes:
                        area_premium_rows += 1
                        total_amount += gap_amount
                        
                        ***REMOVED*** Farm-Key (Name + PLZ fuer Eindeutigkeit)
                        ...
                        ***REMOVED*** Flaechenprämien aggregieren
                    ...
                    ***REMOVED*** Progress
                    if total_rows % 1000 == 0:
                        print(f"Verarbeitet: {total_rows:,} | Flaechenprämien: {area_premium_rows} | Ausgeschlossen: {excluded_rows}")
                
                print(f"Erfolgreich gelesen mit Encoding: {encoding}")
                data_loaded = True
                break  ***REMOVED*** Erfolgreich gelesen
                
        except UnicodeDecodeError:
            print(f"Encoding {encoding} fehlgeschlagen, versuche naechstes...")
            continue
    ...
    ***REMOVED*** Betriebsgroessen schaetzen und kategorisieren
    for farm_key, farm in farm_data.items():
        amount = farm["total_area_premiums"]
        
        ***REMOVED*** Schaetzung: ~300 EUR/ha Direktzahlungen (EU-Standard)
        ...
        ***REMOVED*** Betriebskategorisierung
        if amount >= 200000:
            ...
            farm["farm_category"] = "Grossbetrieb"
            farm["lead_score"] = 9
        elif amount >= 50000:
    ...
    ***REMOVED*** Ergebnisse ausgeben
    ...
    print(f"Flaechenprämien-Zahlungen: {area_premium_rows:,} ({area_premium_rows/total_rows*100:.1f}%)")
    ...
    print(f"Gesamt Flaechenprämien: {total_amount:,.2f} EUR")
    ...
    ***REMOVED*** Top Measure Codes
    ...
    for code, stats in sorted_measures[:10]:
        in_focus = "OK" if code in area_premium_codes else "XX"
    ...
    ***REMOVED*** Top Leads
    ...
    for i, farm in enumerate(sorted_farms[:15], 1):
        ...
        print(f"    Flaechenprämien: {farm['total_area_premiums']:,.0f} EUR")
        print(f"    Geschaetzte Flaeche: {farm['estimated_hectares']:,.0f} ha")
        print(f"    Kategorie: {farm['farm_category']} (Score: {farm['lead_score']}/10)")
        print(f"    Codes: {codes_str} | Zahlungen: {farm['payment_count']}")
        print()
    
    ***REMOVED*** Betriebsgroessen-Verteilung
    ...
    ***REMOVED*** JSON Export
    result_data = {
        ...
        "analysis_focus": "Flaechenprämien fuer Betriebsgroessenschaetzung",
        "statistics": {
    ...
    print()
    print("SUCCESS! PLZ 26XXX-Leads mit praeziser Flaechenbewertung analysiert!")
    premium_leads = len([f for f in farm_data.values() if f['lead_score'] >= 7])
    print(f"Ihre {premium_leads:,} Premium-Leads (Score 7+) sind verkaufsbereit!")
