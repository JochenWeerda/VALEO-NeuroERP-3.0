...
class InventoryEntity {
    constructor(id, name, status, productId, quantity, reservedQuantity, availableQuantity, reorderLevel, reorderQuantity, lastUpdated, createdAt, updatedAt) {
        this.id = id;
        this.name = name;
        this.status = status;
    ...
    static create(command) {
        ...
        const now = new Date();
        return new InventoryEntity(id, command.name ?? '', command.status ?? undefined, command.productId ?? '', command.quantity ?? 0, command.reservedQuantity ?? 0, command.availableQuantity ?? 0, command.reorderLevel ?? 0, command.reorderQuantity ?? 0, command.lastUpdated ?? new Date(), now, now);
    }
    update(command) {
        // Validate command
