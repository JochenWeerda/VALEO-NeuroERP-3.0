...
| FEED-SUP-004 | Bidirektionaler Kundenrezeptur-Kreislauf | SOLL | Kundenrezeptur (eigene Artikelnr., append-only Versionen als verknuepfte Teilmenge einer Ration) mit Freigabe der Optimal-Rezeptur; **Bestellung fixiert immer die freigegebene Version** (Drift-Schutz strukturell, Soll=kg_per_t×Menge, idempotent); **Ruecklauf** der Ist-Mischung (Mahl-/Mischwagen) mit Nachkalkulation je Komponente (delta_kg/delta_pct, append-only) gegen die fixierte Version; unbekannte Ist-Komponente ⇒ 422 statt Raten | VERIFIED | Einkaufs-/DMS-Anbindung der Bestellung und Portal-Rezeptur-Maske folgen | FEED-RECIPE-052 | `test_feeding_recipes_api.py` | FEED-RECIPE-052 |
