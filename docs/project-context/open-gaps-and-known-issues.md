...
- Superglue Self-Host ist mit `INT-SG-035` bis `INT-SG-062` auf den aktuellen Upstream-Runtime-/REST-Vertrag gezogen; CRM-/Masterdata, Artifact-/Idempotenz-/Retry-Pfade, Admin-/Monitoring-/CI-Surfaces sowie Procurement-, Finance-, Logistics-, Agribusiness-, Service- und Analytics-Rollouts liegen jetzt als thin-wrapper Connector-Familien im VALEO-Pfad vor. `INT-SG-061` surfact die verbleibende Live-Readiness fuer fehlende Tenant-Credentials, Platzhalter-Zielsysteme sowie Alerting-/Retention-Policies direkt in API und Admin-UI, und `INT-SG-062` exportiert dafuer jetzt einen konkreten Tenant-Onboarding-Pack mit Secret-Key-Kandidaten und Zielsystem-Feldern. **Operativ offen bleiben** damit nur noch die echte Befuellung je Tenant/Environment: Live-Credentials, produktive Zielsystem-URLs und Ops-seitige Alerting-/Retention-Werte. **Kernel/Finance:** `PostAPInvoice` fuehrt Journal+OP+Outbox im Kernel-Pfad aus (`app/services/ap_invoice_kernel_posting.py`); Execute-Request braucht `human_confirmation: true`. Einkauf: optionale Tenant-Praefixe `TENANT_PO_PREFIX_<SANITIZED_TENANT>`; Duplikat-Bereinigung+Unique per Migration `einkauf_bestellungen_dedupe_unique_20260407`. Finance-Export: zweiter Upload-Weg S3/MinIO (`FINANCE_EXPORT_S3_*`, `finance_export_upload.py`).
