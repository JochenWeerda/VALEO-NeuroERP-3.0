...
- ~~Der Leitstand fuer Agent Ops und Superglue hing nur als breite Sammelseite unter `Agenten-Integration`~~ -> `PCP-019` bis `PCP-021` ziehen den Admin-Unterbereich jetzt nach `packages/frontend-web/src/pages/admin/control-center/` mit dedizierten Seiten fuer `overview`, `superglue` und `agent-ops`; Routing, Aliase und Navigation zeigen auf `admin/control-center`, `admin/control-center/superglue` und `admin/control-center/agent-ops`, waehrend die Legacy-Sammelseite nur noch als Kompatibilitaetseinstieg bestehen bleibt.
- ~~Agent Ops hatte im Leitstand noch keine persistente Read-Model-/Revisionsbasis ueber Neustarts hinweg~~ -> `PCP-022` persistiert Snapshots und History jetzt unter `runtime/agent-ops/state.json` und `runtime/agent-ops/history.jsonl`; `app/agents/agent_ops.py` laedt tenant-spezifische Snapshots wieder ein und surfact den Zustand ueber `GET /api/v1/agents/neuroassist/ops/persistence` sowie die Admin-Seite `admin/control-center/agent-ops`.
- ~~Superglue war im Admin-Unterbereich weiterhin weitgehend lesend und hatte keinen echten Tenant-Onboarding-Flow~~ -> `PCP-023` und `PCP-024` liefern persistente Admin-Overrides (`runtime/superglue/admin-state.json`), neue Mutationsendpunkte fuer Connector-/Policy-Overrides sowie den Wizard `GET/POST /agent/integrations/providers/superglue/tenants/{tenant_id}/wizard` und die neuen Leitstandskarten in `packages/frontend-web/src/pages/admin/control-center/superglue.tsx`.
- ~~Im neuen Kontrollzentrum fehlten noch echte Planung und ein gemeinsames Incident-/Alert-/Escalation-Center~~ -> `PCP-025` und `PCP-026` liefern `app/agents/agent_control_center.py` mit persistenter Planungsdatei `runtime/agent-ops/planning.json`, den Endpunkten `GET/POST /api/v1/agents/neuroassist/ops/planning*` und `GET/POST /api/v1/agents/neuroassist/ops/incidents*` sowie der zentralen Leitstandsseite `packages/frontend-web/src/pages/admin/control-center/index.tsx` fuer Scheduling-Matrix und gemeinsame Agent-/Superglue-Vorfallsteuerung.
