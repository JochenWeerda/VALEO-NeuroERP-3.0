...
## SEC-GITHUB-WARNINGS-CLOSEOUT-001 GitHub-Warnings und rote Gates schliessen - reserviert 2026-07-10

**Owner:** Codex. **Stand:** reserviert 2026-07-10. **Ziel:** offene GitHub-Warnings soweit praktisch behebbar schliessen: rote Actions auf `main` priorisiert (`Quality Gate`, `CI/CD Pipeline`, `Docs Build`), konkrete SonarCloud-Highs pruefen und behebbaren Quellcode fixen, Container-Scan-Warnungen auf Base-Image-/Scan-Scope-Massnahmen herunterbrechen. **Dateibesitz:** `services/*/requirements.txt` fuer pytest/pytest-asyncio-Kompatibilitaet, `app/api/v1/endpoints/silo_cells_readmodel.py`, `app/services/calendar_projection_service.py`, `app/services/gap_pipeline_service.py`, generierte Inventare unter `docs/schnittstellen/endpoint-inventory.md`, `docs/entwickler/service-inventory.md`, `docs/admin/migration-inventory.md`, ggf. CI-/Dockerfile-Dateien nach Befund, `docs/agent-ops/slices/SEC-GITHUB-WARNINGS-CLOSEOUT-001.yaml`. **Abnahme:** relevante lokale Gate-Skripte gruen, fokussierte Py-Compile/Tests, Push auf `main`, GitHub Actions/Alerts erneut geprueft.
