...
| INT-SG-043 | Tenant-/System-Bootstrap fuer echte Superglue-Connectoren | reserviert | Codex | `app/integrations/services/**`, `app/api/v1/endpoints/external_agent_integrations.py`, `scripts/superglue/**`, `tests/test_superglue_*`, `docs/workflows/int-sg-043-superglue-tenant-bootstrap.md`, `docs/cards/neuro-core/INT-SG-043-superglue-tenant-bootstrap.md` | echten Bootstrap fuer Systems, Credentials und Tool-Definitionen pro Tenant schneiden | Upstream-System-/Credential-Shape muss tenant-sicher und driftarm modelliert werden |
| INT-SG-044 | Produktiven Secret-/Credential-Resolver fuer Superglue verdrahten | reserviert | Codex | `app/integrations/services/superglue_secret_resolver.py`, `app/services/secrets_vault.py`, `app/core/config.py`, `tests/test_superglue_*`, `docs/workflows/int-sg-044-superglue-secret-resolver.md`, `docs/cards/neuro-core/INT-SG-044-superglue-secret-resolver.md` | tenant-spezifische Secret-Aufloesung in den Laufzeitpfad statt nur Ops-/Bootstrap-Doku ziehen | keine Shared-Credentials oder stillen Fallbacks im produktiven Multi-Tenant-Betrieb |
| INT-SG-045 | Tool-Lifecycle-Management und Drift-Control | reserviert | Codex | `app/integrations/adapters/superglue/tool_sync.py`, `app/integrations/services/superglue_tool_provisioning.py`, `app/api/v1/endpoints/external_agent_integrations.py`, `tests/test_superglue_tool_*`, `docs/workflows/int-sg-045-superglue-tool-lifecycle.md`, `docs/cards/neuro-core/INT-SG-045-superglue-tool-lifecycle.md` | Create/Update/Archive/Version-Delta fuer echte Tools und Drift-Sicht im Admin-Pfad umsetzen | Lifecycle darf VALEO-Contract-Mapping nicht von Upstream-IDs entkoppeln |
| INT-SG-046 | Run-Result-Normalisierung fuer echte Connector-Runs | reserviert | Codex | `app/integrations/contracts/**`, `app/integrations/services/superglue_execution_service.py`, `app/integrations/services/superglue_execution_journal.py`, `tests/test_superglue_execution_*`, `docs/workflows/int-sg-046-superglue-result-normalization.md`, `docs/cards/neuro-core/INT-SG-046-superglue-result-normalization.md` | echte Upstream-Responses/Fehler in stabile VALEO-Envelopes ueberfuehren | Result-/Error-Semantik darf nicht pro Connector auseinanderlaufen |
| INT-SG-047 | DMS-Connector als erste echte produktive Fachanbindung | reserviert | Codex | `app/integrations/adapters/superglue/document_adapter.py`, `app/integrations/ports/document_port.py`, `tests/test_superglue_document_adapter.py`, `docs/workflows/int-sg-047-superglue-dms-connector.md`, `docs/cards/neuro-core/INT-SG-047-superglue-dms-connector.md` | Pilot-Preview durch echten DMS-Read-/Search-/Reference-Pfad ersetzen | externer DMS-Contract, Dateireferenzen und tenant-spezifische Berechtigungen |
| INT-SG-048 | Partner-EDI-Connector als zweite echte Fachanbindung | reserviert | Codex | `app/integrations/adapters/superglue/edi_adapter.py`, `app/integrations/ports/partner_edi_port.py`, `tests/test_superglue_partner_preview.py`, `docs/workflows/int-sg-048-superglue-partner-edi.md`, `docs/cards/neuro-core/INT-SG-048-superglue-partner-edi.md` | echten Partner-/EDI-Simulations- und Mappingpfad statt Preview liefern | Partner-spezifische Schema-/Fehlerlogik muss sauber versioniert werden |
| INT-SG-049 | CRM-/Stammdaten-Connector produktiv machen | offen | — | `app/integrations/adapters/superglue/customer_profile_adapter.py`, `app/integrations/ports/customer_profile_port.py`, `tests/test_superglue_customer_profile_adapter.py`, `docs/workflows/int-sg-049-superglue-crm-masterdata.md`, `docs/cards/neuro-core/INT-SG-049-superglue-crm-masterdata.md` | echten externen Customer-/Masterdata-Read fuer CRM liefern | Datenschutz, Tenant-Zuordnung und Feldmapping muessen belastbar sein |
| INT-SG-050 | File-Reference-/Artifact-Pfad produktivieren | offen | — | `app/integrations/services/superglue_execution_service.py`, `k8s/superglue/**`, `docker-compose.integration.yml`, `tests/test_superglue_*`, `docs/workflows/int-sg-050-superglue-file-references.md`, `docs/cards/neuro-core/INT-SG-050-superglue-file-references.md` | echte Dateiartefakte und File-References aus Runs in VALEO nutzbar machen | Dateilebenszyklus, Zugriffsschutz und Kostenkontrolle |
| INT-SG-051 | Execute-Gates fuer reale Connector-Writes haerten | offen | — | `app/services/neuro_tool_broker.py`, `app/services/neuro_tool_execution.py`, `app/integrations/services/superglue_execution_service.py`, `tests/test_superglue_broker_integration.py`, `docs/workflows/int-sg-051-superglue-execute-gates.md`, `docs/cards/neuro-core/INT-SG-051-superglue-execute-gates.md` | echte Write-Connectoren nur ueber Approval/Policy/Audit zulaessig machen | kein Direkt-Execute-Bypass ueber Superglue |
| INT-SG-052 | Idempotenz, Replay-Guard und Correlation-Hardening | offen | — | `app/integrations/services/superglue_execution_service.py`, `app/integrations/services/superglue_execution_journal.py`, `tests/test_superglue_execution_*`, `docs/workflows/int-sg-052-superglue-idempotency.md`, `docs/cards/neuro-core/INT-SG-052-superglue-idempotency.md` | doppelte externe Writes und unsaubere Retries technisch verhindern | externe Systeme liefern evtl. keine sauberen Idempotency-Guarantees |
| INT-SG-053 | Quarantaene, Retry und Dead-letter fuer echte Connectorfehler | offen | — | `app/integrations/services/superglue_quarantine.py`, `app/integrations/services/superglue_execution_service.py`, `tests/test_superglue_refresh_and_quarantine.py`, `docs/workflows/int-sg-053-superglue-quarantine-retry.md`, `docs/cards/neuro-core/INT-SG-053-superglue-quarantine-retry.md` | betriebsfaehige Fehlerpfade fuer degradierte Connector-Runs abschliessen | Retry darf keine fachlichen Dubletten erzeugen |
| INT-SG-054 | Admin-Surface fuer Systems, Credentials, Tools und Drift | offen | — | `app/api/v1/endpoints/external_agent_integrations.py`, `packages/frontend-web/src/pages/admin/**`, `tests/test_process_kernel_wave88_external_agent_integrations.py`, `docs/workflows/int-sg-054-superglue-admin-surface.md`, `docs/cards/neuro-core/INT-SG-054-superglue-admin-surface.md` | Betreiberpfade fuer Bootstrap, Drift, letzte Runs und Quarantaene surfacen | keine direkte Fachbedienung oder Secret-Exposition im UI |
| INT-SG-055 | Monitoring, Kosten und Alerting pro Connector/Tenant | offen | — | `app/integrations/services/**`, `k8s/helm/valeo-erp/templates/**`, `ops/superglue/**`, `docs/workflows/int-sg-055-superglue-monitoring.md`, `docs/cards/neuro-core/INT-SG-055-superglue-monitoring.md` | Health um Latenz, Fehlerrate, Kosten und Tenantsicht erweitern | Telemetrie darf PII/Secret-Daten nicht leaken |
| INT-SG-056 | Staging-/CI-Smoke mit echtem Dev-Connector | offen | — | `.github/workflows/**`, `scripts/superglue/**`, `tests/test_superglue_*`, `docs/workflows/int-sg-056-superglue-ci-smoke.md`, `docs/cards/neuro-core/INT-SG-056-superglue-ci-smoke.md` | reproduzierbaren automatisierten Smoke fuer mindestens einen echten Connector liefern | CI-Secrets und externe Testsysteme muessen sauber isoliert werden |
...
| INT-SG-060 | Optionaler breiter Rollout auf Agrar, Service und Analytics | offen | — | `app/integrations/adapters/superglue/**`, `packages/frontend-web/src/pages/**`, `docs/workflows/int-sg-060-superglue-broad-rollout.md`, `docs/cards/neuro-core/INT-SG-060-superglue-broad-rollout.md` | priorisierte Zusatzdomänen nach demselben Connector-Standard erschliessen | nur nach Referenz-Connectoren und Betriebsreife starten |
...
**Von:** Codex
**An:** naechste Session / naechster Agent
**Ziel des Slices:** Den zentralen Frontend-HTTP-Contract so harmonisieren, dass bestehende Payload-first- und `.data`-Zugriffe parallel typisiert bleiben und der globale Frontend-Typecheck wieder grün laeuft.
**Stand:** abgeschlossen
**Erledigt:**
