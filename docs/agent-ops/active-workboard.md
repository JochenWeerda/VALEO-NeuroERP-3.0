...
| SEC-001 | Security Incident P0: Hardcoded Secrets und unsichere Default-Credentials entfernen | reserviert | Codex | `docs/agent-ops/active-workboard.md`, `scripts/mcp_server.py`, `scripts/genxais_prompt_generator_simple.py`, `scripts/test_mcp_api.py`, `alembic.ini`, `app/core/config.py`, `packages/*/docker-compose.yml` | echte Repo-Secrets entfernen, Default-Credentials auf env-only oder explizite Platzhalter ziehen, Doku nachziehen | dritte Security-Triage fehlt |
