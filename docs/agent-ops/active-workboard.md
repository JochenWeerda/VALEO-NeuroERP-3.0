...
## FEED-CORE-016 Tiergruppenprofile, Gueltigkeit, Historie und native ObjectPage - reserviert 2026-07-15

**Owner:** Codex. **Stand:** reserviert (Claim). Vertikaler Ausbau von `feeding_groups` fuer FEED-HERD-001/002 und FEED-WP-011 bis 020: typisierte Milchvieh-/Jungviehprofile, Traechtigkeits- und Milchinhaltsstoffparameter, Risiko, Gueltigkeitsintervall, optimistische Revision und unveraenderliche Parameterhistorie; tenant- und Business-Grant-sichere Create/List/Get/Update/History-Vertraege; native Meridian-ObjectPage `agrar/feeding-group` ueber ScreenDefinition → RenderPlan → UniversalMaskRuntime → UniversalMaskRenderer. **Entscheidungsstufe:** Significant innerhalb des bestehenden Agrar-/Rations-Optimization-Containers; Domain Pack und Proposed ADR werden im Slice aktualisiert. **Dateibesitz:** additive Alembic-Migration nach `feed_core_business_20260715`, Gruppen-Domainmodul, `rations_lifecycle`-Service/API, fokussierte Backendtests, `app/core/screen_definitions.py`, schmale Portal-Route/Domain-Overlays, Frontendtests, Agrar-Domain-Pack, ADR, Impact Note, Feeding-Kapitel/Traceability, Agent-Handbuch, Slice-YAML und Workboard. **TDD:** zuerst fehlschlagende Domain-/API-/Migration-/Screen-Vertraege; danach kleinste Implementierung und Regression.
