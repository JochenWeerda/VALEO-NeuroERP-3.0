...
| INT-SG-001 | Superglue Contract- und Settings-Basis | reserviert | Codex | `docs/architecture/superglue-integration-implementation-plan.md`, `app/integrations/contracts/**`, `app/core/config.py`, `tests/test_superglue_contracts.py` | zentrale Typen, Result Envelope und `SUPERGLUE_*`-Settings ohne Dopplung einfuehren | Abstimmung mit bestehendem Config-/Secret-Schema |
| INT-SG-002 | Superglue Client + Egress Guard | offen | — | `app/integrations/adapters/superglue/client.py`, `app/integrations/services/integration_circuit_breaker.py`, `app/core/outbound_security.py` | reinen Transportclient mit Timeout/Retry-Basis und SSRF-/Allowlist-Pruefung aufbauen | Zielhost-/Auth-Modell pro Deployment noch offen |
