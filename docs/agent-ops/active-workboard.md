...
## RATIONS-UX-MOBILE-013 Mobile Fuetterungsdokumentation - reserviert

**Owner:** Codex. **Stand:** reserviert 2026-07-12. **Ziel:** Eigene touch-optimierte Route fuer aktive Ration, "Jetzt fuettern", Ist-Mengen, Restfutter/TM-Schnellmessung und Schuettelbox; schreibt in das F1-Feeding-Log, startet keinen Solver und bleibt auf Smartphone ohne horizontalen Body-Scroll. **Dateibesitz:** neue mobile Seite/Route und Tests unter `packages/frontend-web`, Rations-API-Typen nur falls erforderlich, Workboard, Slice-YAML, Rations-Masterplan und generierte Route-/Handbuch-Artefakte. **Abnahme:** Kernflow auf 390px-Viewport bedienbar, Persistenzvertrag korrekt, TypeScript/ESLint/Build und fokussierter Playwright-Smoke gruen. **Risiken:** Keine Duplikation der Rationsberechnung; mobile Seite nutzt F1-API und vorhandene Rationsdaten.
