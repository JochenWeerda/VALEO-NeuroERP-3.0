...
## FEED-EDITOR-024 Min/Max-Grenzen in der Ration + benannte Grenzkonflikte - in Arbeit 2026-07-16

**Owner:** Claude. **Stand:** in Arbeit (Claim). Fortsetzung Inkrement 2 (Requirements FEED-RAT-001-Rest „Min/Max direkt in der Ration", FEED-OPT-004-Vertiefung „konfliktverursachende Grenzen benennen"; Arbeitspaket FEED-WP-072-Kern): Draft-Komponenten erhalten optionale `min_kg_fm`/`max_kg_fm`; die deterministische Bewertung prüft VOR jedem Solverlauf strukturelle Grenzkonflikte und benennt die Verursacher — `bounds_conflict` je Position (min>max, critical), `amount_outside_bounds` (Menge verletzt eigene Grenze, high), `min_sum_exceeds_dmi_band` (Summe der TM-Minima sprengt das TM-Band, critical, mit beteiligten Futtermitteln im Text). Editor bekommt Min/Max-Spalten; Befund→Ursache-Navigation greift auch für Grenzkonflikte. Ergänzt die bestehende Solver-Erklärschicht (explizite infeasible-Rationale + Lösungsvorschläge bereits vorhanden — Traceability-IST wird korrigiert). **TDD:** Verträge zuerst rot. **Dateibesitz:** `ration_draft.py` (additiv), `feeding_ration_editor.py` (Schema additiv), Tests (erweitert), `RationEditor.tsx` + Vitest (erweitert), Traceability, Slice-YAML, Workboard.
