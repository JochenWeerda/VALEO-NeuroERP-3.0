...
## FEED-ADVICE-UX-011 Offene Zielbildpruefung der Rationsberatung - reserviert 2026-07-14

**Owner:** Codex. **Stand:** reserviert. **Ziel:** Die bestehende 242-kB-Agrar-Spezialmaske ohne Bestandsschutz gegen eine native Meridian-ScreenDefinition und ein fachlich begruendetes Hybridmodell bewerten; das Zielbild nach Aufgabenfluss, Informationsdichte, Fehlerpraevention, Tastatur-/Touch-Bedienung, Barrierefreiheit, Performance und zentraler Wartbarkeit entscheiden und als eigenstaendiges VALEO-Design prototypisch umsetzen. **Dateibesitz:** `packages/frontend-web/src/pages/futtermittel/rationsoptimierung.tsx`, relevante `packages/frontend-web/src/components/mask-builder/**`, neue native Rations-ScreenDefinition/Tests, UX-/ADR-Dokumentation, Agrar-Domain-Pack, Workboard. **Abnahme:** dokumentierte Variantenbewertung; keine kopierte Fodjan-Informationsarchitektur oder Markenassets; produktiver Pfad ueber `ScreenDefinition -> RenderPlan -> useUniversalMaskRuntime -> UniversalMaskRenderer` oder explizit begruendete Erweiterung; focused Typecheck, Vitest, ESLint, Browser-/A11y-Smoke und Architektur-Gates gruen. **Risiken:** Solver-Interaktion und hochdichte Rationstabelle koennen neue generische Renderer-Primitiven benoetigen; bestehende Fachfunktionen duerfen bei der Migration nicht verloren gehen.
