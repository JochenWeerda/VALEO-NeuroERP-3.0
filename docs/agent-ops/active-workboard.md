...
## FEED-SPEC-REFERENCE-038 Architektur-Referenzwerk 00-17 + 240-Pakete-Arbeitsprogramm - in Arbeit 2026-07-15

**Owner:** Codex. **Stand:** in Arbeit (Claim). Explizite Auftraggeberentscheidung vom 2026-07-15 hebt die Foundation-Empfehlung gegen ein 300-500-Seiten-Vorabwerk auf. Ziel ist ein querverlinktes Fachkonzept auf Architektenniveau: Vision/Glossar/Lastenheft/Fachkonzept, DDD samt Event Storming und UML, logisches/physisches Datenmodell, vollstaendige API-Spezifikation, Masken- und Workflowkatalog, versioniertes Regelwerk, KI-Agentenvertraege, Integrationen, Testkatalog, Migration/Rollout/Traceability sowie 200-300 vertikale Arbeitspakete mit Abnahme, Abhaengigkeit, DoD, Test und Aufwand. Pflegevertrag: normativ vs. informativ kennzeichnen, Code-/OpenAPI-/Migration-Quellen verlinken, keine zweite Wahrheit fuer Formeln erzeugen, Drift-Metadaten je Kapitel. **Dateibesitz:** neue nummerierte Dateien `docs/specs/feeding/00-*.md` bis `17-*.md`, `docs/specs/feeding/reference-maintenance.md`, `docs/specs/feeding/work-packages/*.md`, Feeding-README (Zielbildentscheidung), Slice-YAML, Workboard. Bestehende `requirements-traceability.md`, `implementation-plan.md`, produktive Code-/API-/Migration-Dateien und FEED-CORE-015-Dateien bleiben ausserhalb dieses Slice.

**Zwischenstand 2026-07-15:** Pflegevertrag sowie Kapitel 00, 01 und 04 bis 13 sind als acht reviewbare Tranches geliefert (4.320 Zeilen Referenztext). Enthalten sind DDD/ERD, vollstaendiger Zieltabellen- und API-Katalog, 22 eigenstaendige Meridian-Masken, 15 End-to-End-Workflows, GfE-/DLG-Regelgovernance mit NRC-Gate, UI/UX-Konzept, sechs sicherheitsbegrenzte Fachagenten, Integrationsarchitektur und exakt 200 stabile Abnahmetest-IDs. Offen im selben Slice: eigenstaendige Kapitel 02/03 sowie 14 bis 17, 240 vertikale Arbeitspakete, abschliessende Querverlinkung und Drift-/Vollstaendigkeitsabnahme. FEED-CORE-015 bleibt unberuehrt parallel in Arbeit.
...
**Owner:** Codex (Uebernahme vom pausierten Claude-Code-Chat auf ausdruecklichen Auftrag). **Stand:** in Arbeit. Erstes Slice von Inkrement 1 des Umsetzungsplans (`docs/specs/feeding/implementation-plan.md`; Requirements FEED-BUS-001/002, FEED-RBAC-003): Aggregate FeedingBusiness/FarmSite/Herd im Schema `domain_agrar` (additive Migration, `business_partner_id` als CRM-Bruecke gemaess Identitaetsstandard), `feeding_groups` erhaelt nullable `business_id`/`herd_id`-FKs, Backfill „Eigener Betrieb" je Tenant, Betriebs-Grants-Tabelle als zweite Rechtestufe (subject+scope+valid_until) mit `require_business_access`-Dependency, Router `/api/v1/agrar/rations-optimization/feeding/*` (Rollen aus authz.py; Grant-Verwaltung Admin-Level), fokussierte 403-/Integrationstests, Betriebsliste (mask-builder ListReport) + API-Client. TDD-Haertung vor Abnahme: tenant-sicheres Upsert, Business-/Site-/Herd-Invarianten, append-only Grant-Widerruf und Durchsetzung der Business-Grants an Fachendpunkten. **Dateibesitz:** Migration `feed_core_business_20260715`, `app/services/feeding_business_service.py` (neu), `app/api/v1/endpoints/feeding_core.py` (neu), `app/agrar/rations/authz.py` (additiv), `tests/test_feeding_business_core.py` (neu), `lib/api/feeding-business.ts` + `pages/futtermittel/fuetterungsbetriebe.tsx` (neu), Traceability-Update, Slice-YAML, Workboard.
