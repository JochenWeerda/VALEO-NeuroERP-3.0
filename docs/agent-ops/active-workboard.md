...
**Owner:** Claude (disjunkt zu Codex' FEED-ACT-030-WIP). **Stand:** in Arbeit (Claim). Abschluss Inkrement 6 gemaess Slice-Spec 049 (Requirements FEED-INT-004; FEED-INT-003 bleibt BLOCKED/Partnervertrag): Zentrales Event-Modul `app/agrar/rations/events.py` (Schema 1.0: schema_version/event_id/event_type/aggregate_id/timestamp/payload; geschlossene Typliste, unbekannter Typ → ValueError) extrahiert Codex' Inline-Muster aus dem Plan-Service; Emission **in derselben Transaktion** (genau-einmal mit dem fachlichen Commit) an den drei WIP-freien Punkten: `feeding.ration.version.activated` (Lifecycle-Transition inkl. superseded-Liste), `feeding.analysis.released` (Analyse-Freigabe), `feeding.import.quarantined` (Monitor: fehlerhafte Payloads + externe Konflikte wie Mixer-Rueckmeldungen). Emission an `feeding.actuals/measure/deviation` folgt nach Codex' 030-Commit (Dateikollision vermieden). Konsumenten (Aufgaben/Benachrichtigung) sind 032-Scope. **TDD:** Contract- und Emissions-Journeys zuerst rot. **Dateibesitz:** `app/agrar/rations/events.py` (neu), `rations_lifecycle_service.py` + `feeding_feed_analysis_service.py` + `feeding_import_monitor_service.py` (additiv Emission), `tests/test_feeding_events.py` (neu), Traceability, Slice-YAML, Workboard.
