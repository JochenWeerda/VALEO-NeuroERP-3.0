...
| INT-SG-004 | Superglue an Neuro Tool Broker anbinden | offen | — | `app/integrations/services/superglue_execution_service.py`, `app/integrations/services/superglue_capability_service.py`, `app/services/neuro_tool_broker.py`, `app/services/neuro_tool_execution.py` | Execution nur ueber bestehende Broker-/Policy-/Approval-Kette | Ueberschneidung mit Neuro-Core-Brokerpfaden sauber koordinieren |
| INT-SG-005 | Superglue Deployment / Ops / Admin-Link | offen | — | `docker-compose*.yml`, Ops-Doku, optional Admin-Link im Frontend | eigenstaendiger Container, eigene DB, Admin-Link statt Embedding | Port-/Auth-/Network-Policy finalisieren |
| INT-SG-006 | Erste Pilotanbindung ueber Superglue | offen | — | konkrete Adapterdatei je Zielsystem + Tests + Slice-Doku | eine reale, nicht-kritische Pilotintegration ueber denselben Standardpfad liefern | Zielsystemauswahl und Betriebsfreigabe offen |
