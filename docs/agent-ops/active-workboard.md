...
| INT-SG-035 | Upstream Runtime-Contract fuer Superglue self-host angleichen | reserviert | Codex | `docker-compose.integration.yml`, `docker-compose.integration.edge.yml`, `k8s/superglue/**`, `k8s/helm/valeo-erp/templates/superglue-*.yaml`, `k8s/helm/valeo-erp/values.yaml`, `scripts/superglue/**` | Compose-, K8s- und Helm-Pfad auf den aktuellen Upstream-Env-/Port-/Storage-Contract ziehen | Upstream `main` kann weitere Env-Drift enthalten |
| INT-SG-036 | Superglue REST- und Health-Mapping auf `/v1` ziehen | reserviert | Codex | `app/integrations/adapters/superglue/client.py`, `app/integrations/adapters/superglue/tool_sync.py`, `tests/test_superglue_client.py`, `tests/test_superglue_tool_sync.py`, `tests/test_process_kernel_wave88_external_agent_integrations.py` | Health, Tool-Katalog und Sync auf den aktuellen OSS-REST-Contract umstellen | GraphQL-Fallback aus alter Annahme muss konsistent entfernt oder degradierbar gemacht werden |
| INT-SG-037 | Tool-Execution und Run-Tracking gegen Upstream-Runs haerten | reserviert | Codex | `app/integrations/services/superglue_execution_service.py`, `app/integrations/services/superglue_execution_journal.py`, `app/integrations/services/superglue_capability_service.py`, `app/services/neuro_tool_broker.py`, `tests/test_superglue_execution_guardrails.py`, `tests/test_superglue_broker_integration.py`, `tests/test_superglue_refresh_and_quarantine.py` | Execution-Service auf `/v1/tools/{toolId}/run` und Run-Envelope umstellen | Async-/Sync-Run-Semantik muss mit VALEO-Envelope vereinheitlicht werden |
| INT-SG-038 | Tenant-/System-Bootstrap und Tool-Provisioning fuer Superglue nachziehen | reserviert | Codex | `app/integrations/adapters/superglue/*.py`, `app/integrations/services/superglue_secret_resolver.py`, `scripts/superglue/bootstrap-secrets.*`, `scripts/superglue/**`, `tests/test_superglue_*` | Bootstrap auf Upstream-Tenant-/System-/Credential-Pfade erweitern und Pilotadapter an echte Tool-/Run-Pfade anbinden | Upstream-OSS bietet nicht jeden fachlichen Endpoint direkt; Mapping muss ueber Tools erfolgen |
| INT-SG-039 | Live-Smoke und Runtime-Verifikation gegen echten Superglue-Container | reserviert | Codex | `scripts/superglue/**`, `.github/workflows/superglue-infra.yml`, `.github/workflows/superglue-deploy.yml`, `tests/**`, `docs/workflows/int-sg-039-superglue-runtime-smoke.md`, `docs/cards/neuro-core/INT-SG-039-superglue-runtime-smoke.md` | Reale Smoke-Pfade fuer Health, Tool-Listing und Tool-Run gegen Upstream-Container absichern | Netzwerk-/Secret-Abhaengigkeit in CI muss kontrolliert bleiben |
| INT-SG-040 | Zielstruktur- und Doku-Alignment fuer Superglue Integration abschliessen | reserviert | Codex | `docs/architecture/superglue-integration-implementation-plan.md`, `docs/project-context/open-gaps-and-known-issues.md`, `docs/agent-ops/active-workboard.md`, `app/integrations/**`, `docs/workflows/int-sg-040-superglue-target-structure.md`, `docs/cards/neuro-core/INT-SG-040-superglue-target-structure.md` | Soll-/Ist-Dokumentation, fehlende `__init__`-/Port-Struktur und Folgeluecken auf belastbaren Abschlussstand ziehen | Kein stilles Umbauen ohne Runtime-Nutzen |
