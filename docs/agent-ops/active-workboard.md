...
**UIX-063 reserviert 2026-07-07 (Owner Codex):** Planungskalender v1 als Zeitprojektion. Dateibesitz: `alembic/versions/*calendar_items*`, `app/services/calendar_projection_service.py`, `app/api/v1/endpoints/planung_kalender.py`, `app/core/screen_definitions.py`, `packages/frontend-web/src/components/mask-builder/renderers/CalendarRenderer.tsx`, `config/saison_kalender.yaml`, `docs/agent-ops/slices/UIX-063.yaml` sowie notwendige Tests/Doku. Ziel: `calendar_items`-Read-Model, 5 idempotente Projektoren, tokenbasierter read-only ICS-Feed und native `planung/kalender`-Cockpit-SD mit Layern, Fristenband und Klick-Durchstich. Abnahme: Projektor-/API-Tests, Renderer-Vitest, Playwright Kalender-Layer-Durchstich, Runtime-Sweep 0x5xx.
