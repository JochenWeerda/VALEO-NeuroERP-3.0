...
## RATIONS-CTRL-009..012 Fuetterungscontrolling-Regelkreis nach DLG 01|2025 - reserviert

**Owner:** Codex. **Stand:** reserviert 2026-07-11. **Ziel:** Folgewelle F1 vollstaendig umsetzen: persistentes Fuetterungsprotokoll je Gruppe/Tag, SOLL/IST-Mischgenauigkeit und TM-Verzehr, PennState-Schuettelbox/peNDF-Abgleich sowie IOFC-/Temperatur-Controlling mit sichtbarem Regelkreis Planung -> Fuettern -> Kontrolle -> Anpassung. **Dateibesitz:** pp/agrar/rations/control/**, pp/api/v1/endpoints/rations_optimization.py, zugehoerige Alembic-Migration, packages/frontend-web/src/lib/api/rations-optimization.ts, packages/frontend-web/src/pages/futtermittel/rationsoptimierung.tsx, fokussierte Tests, OpenAPI/Agent-Handbuch, Slice-YAML, Workboard und Rations-Doku. **Abnahme:** Protokoll persistiert tenant-isoliert; Handrechnungen fuer Mischgenauigkeit/TM-Verzehr/IOFC stimmen; Schuettelbox liefert peNDF-SOLL/IST-Ampel; UI zeigt SOLL/IST/Anpassung und Zeitreihe; pytest, tsc, eslint, build und Drift-Gates gruen. **Risiken:** DLG-Leitplanken nicht als unbelegte Messpraezision ausgeben; bestehende uncommittete F1-Vorarbeit wird erhalten und integriert.
