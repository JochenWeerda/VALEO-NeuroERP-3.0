...
| INT-SG-009 | Superglue Sync-Snapshot-Persistenz und Refresh | offen | ? | `app/integrations/adapters/superglue/tool_sync.py`, `app/api/v1/endpoints/external_agent_integrations.py`, `app/core/config.py` | Sync-Stand persistieren und manuellen Refresh fuer Admins anbieten | produktive Scheduler-/Cron-Anbindung spaeter entscheiden |
| INT-SG-010 | Superglue Config-Summary und Admin-Aktionspfad | offen | ? | `app/api/v1/endpoints/external_agent_integrations.py`, `packages/frontend-web/src/pages/admin/agenten-integration.tsx`, Frontend-Test | maskierte Config-Summary und klare Admin-Next-Steps surfacen | keine Secret-Werte im UI leaken |
| INT-SG-011 | Superglue-Quarantaene fuer degradierte Aufrufe | offen | ? | `app/integrations/services/superglue_quarantine.py`, `app/integrations/services/superglue_execution_service.py`, `app/api/v1/endpoints/external_agent_integrations.py` | degradierte/fallbackende Superglue-Calls als Quarantaene-Eintraege festhalten | Persistenz zuerst lokal/dateibasiert |
| INT-SG-012 | Superglue Audit-/Security-Bridge | offen | ? | `app/integrations/services/superglue_execution_service.py`, `app/services/security_observability.py`, optional Audit-Bruecke | Superglue-Execution an Security-Events und Audit-Metadaten anbinden | DB-unabhaengig und fehlertolerant halten |
| INT-SG-013 | Zweiter read-only Superglue-Pilotadapter | offen | ? | `app/integrations/ports/partner_adapter_port.py`, `app/integrations/adapters/superglue/edi_adapter.py`, Tests | zweiten read-only Preview-Adapter fuer Partner/Legacy liefern | kein produktiver Write-Pfad |
| INT-SG-014 | Superglue Admin-UX fuer Refresh und Quarantaene | offen | ? | `packages/frontend-web/src/pages/admin/agenten-integration.tsx`, `packages/frontend-web/src/__tests__/pages/admin/agenten-integration.test.tsx` | Refresh, Config-Hinweise und Quarantaene in der Admin-Seite surfacen | bestehende Admin-Seite nicht ueberladen |
