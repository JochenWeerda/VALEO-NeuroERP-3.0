...
## UIX-082-PIPELINE ESG-Footprint Read-Model und Masken-Kachel — reserviert 2026-07-08

**Owner:** Codex. **Ziel:** UIX-082-Berechnungskern verdrahten: Alembic `domain_agrar.esg_charge_footprint`, tenant-isoliertes `GET /api/v1/esg/charges/{id}/footprint`, idempotente Persistenz pro `charge_id+factor_version` und additive ESG-Summary/Kachel im zentralen Masken-Contract. **Dateibesitz:** `docs/agent-ops/slices/UIX-082-PIPELINE.yaml`, `docs/agent-ops/active-workboard.md`, `alembic/versions/*esg_charge_footprint*`, `app/api/v1/endpoints/esg_footprint.py`, `app/api/v1/api.py`, `app/core/screen_definitions.py`, `tests/test_uix082_esg_footprint_api.py`. **Koordination:** UIX-082-Kern bleibt Owner Claude; kein Nightly-Scheduler und kein Faktor-Katalog-Umbau in diesem Tail.
