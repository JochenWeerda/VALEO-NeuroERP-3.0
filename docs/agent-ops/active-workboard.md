...
**Von:** Claude
**Owner:** Claude
**Stand:** Phasen A–D abgeschlossen 2026-06-07 (Branch `feat/kim-crm-360`; Konsolidierung kunden-cockpit als Folgescope)
...
**Offene Risiken / Folgescope:** Konsolidierung KIM ↔ `kunden-cockpit` (klassisch belassen, nicht entfernt — Deep-Links/Tests). Belege/OP in DEV leer → reale Leerzustände bis Verkaufsdaten existieren (Mapping verifiziert). `ANTHROPIC_API_KEY` ohne Guthaben → NeuroAI engine='fallback'; Admin kann auf Ollama/OpenRouter wechseln. Direktanlage OP/Beleg aus dem Cockpit ist Folgescope (Lesen funktioniert). Vorbestehende, nicht-KIM CSS-Build-Warnung (font-stack) + 3 Analytics-Routing-Lücken bleiben rot. Merge `feat/kim-crm-360` → main steht aus (Nutzerfreigabe).
