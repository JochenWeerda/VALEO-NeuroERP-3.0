...
**UIX-062 reserviert 2026-07-07 (Owner Codex):** Collab-Rail v1 fuer datensatzgebundene Notizen und @-Mentions. Dateibesitz: `alembic/versions/*entity_notes*`, `app/api/v1/endpoints/collab_notes.py`, relevante Infrastrukturmodelle, `packages/frontend-web/src/components/mask-builder/renderers/WorkflowPanelRenderer.tsx`, `packages/frontend-web/src/lib/api/collab.ts`, `docs/agent-ops/slices/UIX-062.yaml` sowie notwendige Tests/Doku. Ziel: `domain_shared.entity_notes`, tenant-isolierte CRUD-/Soft-Delete-API, Mention->Inbox-Benachrichtigung, additive `contextRailSections` im Builder und Rail-Renderer-Abnahme.
