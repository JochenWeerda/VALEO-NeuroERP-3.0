...
## FEED-CORE-017 Naehrstoffdefinitionen, Einheiten, FM-TM und Rundung - reserviert 2026-07-15

**Owner:** Codex. **Stand:** reserviert (Claim). Persistierte und versionierte `NutrientDefinition`-/`UnitDefinition`-Referenzdaten, dimensionssichere Decimal-Umrechnung fuer Frischmasse/Trockenmasse und explizite Rundungsregeln werden als zentrale Agrar-Domainstrecke umgesetzt. Der Slice umfasst additive Migration/Seeds, Domain- und Property-Tests, typisierte Read-/Conversion-API, RBAC/Tenant-Isolation sowie eine native Meridian-Referenzansicht ueber ScreenDefinition → RenderPlan → UniversalMaskRuntime → UniversalMaskRenderer. **Entscheidungsstufe:** Significant innerhalb des bestehenden Agrar-/Rations-Optimization-Containers; Proposed ADR, Domain Pack, Impact Note, Feeding-Fachkonzept, Traceability, Handbuch und Architekturindex werden mitgefuehrt. **TDD:** zuerst fehlschlagende Property-, Boundary-, Migration-, API- und Screen-Vertraege; danach kleinste Implementierung und Regression.
