...
| SEC-002 | Security Incident P1: Zentrale lokale Secret-Pflege ueber Vault + Keyring | reserviert | Codex | `docs/agent-ops/active-workboard.md`, `docs/workflows/sec-002-local-secret-management.md`, `docs/cards/security/SEC-002-local-secret-management.md`, `app/services/secrets_vault.py`, `scripts/security/README.md`, `scripts/security/secret_store.py`, `tests/test_audit_hash_chain.py` | vorhandenen Vault-Service um optionales OS-Keyring erweitern und per CLI nutzbar machen | Keyring-Package ggf. optional |
