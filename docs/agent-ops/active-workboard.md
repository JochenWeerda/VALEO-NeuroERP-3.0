...
## DESIGN-GAPS-SWEEP-002 Vorbestehende Frontend-Luecken schliessen (axe-Kontrast, Portal-Aliase, Vitest-Harness, Alt-AppShell) - in Arbeit (Claim 2026-07-14)

**Owner:** Claude. **Stand:** geclaimt 2026-07-14. Schliesst die in `docs/design/frontend-design-skill-audit.md` dokumentierten vorbestehenden Rot-Staende auch fremder Workstreams: (1) theme-bewusste `text-status-*`-Utilities + Statusfarben-Sweep (`text-green-600` u. a., axe-Rot auf /agrar /finance /lager); (2) 5 Portal-Alias-Luecken (routing-integrity); (3) QueryClient-Testharness der Universal-Pilot-Tests (6 rote Vitests); (4) ungenutzte Alt-AppShell `components/layout/AppShell.tsx` entfernen.
