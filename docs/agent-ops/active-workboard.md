...
| INT-SG-003 | Superglue Provider- und Manifest-Sync | reserviert | Codex | `app/integrations/adapters/superglue/tool_sync.py`, `app/core/external_agent_catalog.py`, `app/api/v1/endpoints/external_agent_integrations.py`, `tests/test_superglue_tool_sync.py`, `tests/test_process_kernel_wave88_external_agent_integrations.py` | `provider_key="superglue"` in bestehende Provider-Sicht integrieren, ohne zweite Registry | Mapping Superglue-Katalog -> VALEO-Manifest muss stabil definiert werden |
| INT-SG-004 | Superglue an Neuro Tool Broker anbinden | reserviert | Codex | `app/integrations/services/superglue_execution_service.py`, `app/integrations/services/superglue_capability_service.py`, `app/services/neuro_tool_broker.py`, `app/services/neuro_tool_execution.py`, `tests/test_superglue_broker_integration.py` | Execution nur ueber bestehende Broker-/Policy-/Approval-Kette | Ueberschneidung mit Neuro-Core-Brokerpfaden sauber koordinieren |
| INT-SG-005 | Superglue Deployment / Ops / Admin-Link | reserviert | Codex | `docker-compose.integration.yml`, `docs/architecture/superglue-integration-implementation-plan.md`, `packages/frontend-web/src/pages/admin/agenten-integration.tsx`, `packages/frontend-web/src/__tests__/pages/admin/agenten-integration.test.tsx` | eigenstaendiger Container, eigene DB, Admin-Link statt Embedding | Port-/Auth-/Network-Policy finalisieren |
| INT-SG-006 | Erste Pilotanbindung ueber Superglue | reserviert | Codex | `app/integrations/ports/**`, `app/integrations/adapters/superglue/document_adapter.py`, `tests/test_superglue_document_adapter.py` | eine reale, nicht-kritische Pilotintegration ueber denselben Standardpfad liefern | Zielsystemauswahl und Betriebsfreigabe offen |
| INT-SG-007 | Superglue Tenant-Secret-Resolution | reserviert | Codex | `app/integrations/services/superglue_secret_resolver.py`, `app/services/secrets_vault.py`, `app/core/config.py`, `tests/test_superglue_secret_resolver.py` | tenant-spezifische Credentials fuer Superglue aufloesen und Shared-Tokens aus produktivem Pfad druecken | Key-Namensschema und Rollout fuer bestehende Tenants festziehen |
| INT-SG-008 | Superglue Sync- und Health-Observability | offen | ? | `app/integrations/adapters/superglue/tool_sync.py`, `app/api/v1/endpoints/external_agent_integrations.py`, optional Admin-Frontend | Sync-Status, letzte Fehler und Provider-Health fuer Admins surfacen | echte Health-Checks gegen laufende Instanz nur bei erlaubter Konfiguration |
