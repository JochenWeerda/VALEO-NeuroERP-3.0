...
| INT-SG-009 | Superglue Sync-Snapshot-Persistenz und Refresh | reserviert | Codex | `app/integrations/adapters/superglue/tool_sync.py`, `app/api/v1/endpoints/external_agent_integrations.py`, `app/core/config.py`, `tests/test_superglue_refresh_and_quarantine.py` | Sync-Stand persistieren und manuellen Refresh fuer Admins anbieten | produktive Scheduler-/Cron-Anbindung spaeter entscheiden |
