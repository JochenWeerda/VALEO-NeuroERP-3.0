***REMOVED*** Inventory-Service Betriebsprofile
...
***REMOVED******REMOVED*** Globale Steuerung
...
***REMOVED******REMOVED*** Modulweise Overrides
...
***REMOVED******REMOVED*** Beispiel-Setups
***REMOVED******REMOVED******REMOVED*** Lokaler Testbetrieb (ohne externe Abhängigkeiten)
```env
...
***REMOVED******REMOVED******REMOVED*** Produktivbetrieb
```env
...
***REMOVED******REMOVED******REMOVED*** Selektive Aktivierung des EventBus bei ansonsten testnaher Umgebung
```env
INVENTORY_GLOBAL_OPERATION_MODE=test
INVENTORY_MODULE_MODE_OVERRIDES=event_bus=real
```

***REMOVED******REMOVED*** Implementierungsdetails
