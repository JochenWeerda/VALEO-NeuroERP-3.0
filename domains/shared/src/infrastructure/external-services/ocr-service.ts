/**
 * VALEO NeuroERP 3.0 - Shared Infrastructure - OCR Service
 *
 * Optical Character Recognition service for document processing
 * Basic implementation for invoice document text extraction
 */

// Temporary interface definitions - should be moved to shared types
export interface DocumentId extends String {}
...
export class BasicOCRService implements OCRService {
  ...
  async processDocument(documentPath: string): Promise<OCRData> {
    const documentId = (globalThis.crypto?.randomUUID() || `${Date.now()}-${Math.random().toString(16).slice(2)}`) as DocumentId;
