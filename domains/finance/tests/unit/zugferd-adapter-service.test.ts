...
import { ZUGFeRDAdapterApplicationService, createZUGFeRDAdapterService } from '../../src/application/services/zugferd-adapter-service';
...
describe('ZUGFeRDAdapterApplicationService', () => {
  let service: ZUGFeRDAdapterApplicationService;
  ...
  beforeEach(() => {
    ...
    service = createZUGFeRDAdapterService(dependencies);
  });
  ...
  describe('ingestEInvoice', () => {
    it('should successfully ingest a valid PDF e-invoice', async () => {
      ...
      // Act
      const result = await service.ingestEInvoice(
        testFile,
        'test-invoice.pdf',
        'tenant-1',
        { detectProfile: true }
    ...
    it('should handle processing errors gracefully', async () => {
      ...
      // Act & Assert
      await expect(service.ingestEInvoice(
        testFile,
        'error-invoice.pdf',
        'tenant-1'
