...
| SQL Migration Runner | `tools/migration/run_sql_migration.ts` | Executes raw SQL migrations/seeds against a Postgres database using shared pool helper. | Node 18+, `ts-node` (local or via `npx`), `pg` workspace dependency | `npx ts-node tools/migration/run_sql_migration.ts --file migrations/sql/crm/001_crm_schema.sql` (URL: `--url` / `--env`, sonst Fallback `ERP_DATABASE_URL` → `DATABASE_URL` → `CRM_DATABASE_URL`) |
...
- **ERP-/Finanz-SQL & Mandanten:** [docs/erp-finanz-multitenancy.md](../../docs/erp-finanz-multitenancy.md), [migrations/sql/erp/README.md](../../migrations/sql/erp/README.md).
...
- **DB-URL für SQL-Runner:** Ohne `--url` / `--env` wird nacheinander `ERP_DATABASE_URL`, `DATABASE_URL`, `CRM_DATABASE_URL` gewählt (erste gesetzte, nicht-leere Variable). Für CRM-Integrationstests kann weiter explizit `CRM_DATABASE_URL` gesetzt werden.
- **`.env` für den SQL-Runner:** Zuerst **Repo-Wurzel-`.env`**, dann (falls anderer Pfad) **`.env` im aktuellen Arbeitsverzeichnis** (`override`), damit Migrationen auch von Unterverzeichnissen aus Zugriff auf die Datenbank haben.
