...
def _make_dn_row(status: str, sales_order_id: str | None = ORDER_ID):
    row = MagicMock()
    data = {"status": status, "sales_order_id": sales_order_id,
            "delivery_note_number": "VK-2026-001", "customer_id": "CUST-1"}
    row.__getitem__ = lambda s, k: data[k]
    row.get = lambda k, d=None: data.get(k, d)
    row._mapping = data
    row.keys = lambda: data.keys()
    # Allow dict() to work on the mock
    row.__iter__ = lambda s: iter(data.keys())
    return row


def _mock_db_for_deliver(dn_row, order_fully_delivered: bool):
    db = MagicMock()
    ...
    def execute_side(sql, params=None):
        ...
        if "SELECT * FROM domain_sales.delivery_notes WHERE id" in sql_str:
            result.mappings.return_value.first.return_value = dn_row
        elif "SELECT * FROM domain_sales.delivery_note_positions" in sql_str:
            result.mappings.return_value.all.return_value = []
        elif "SalesPostingService" in sql_str or "book_warenabgang" in sql_str:
            pass
        else:
...
class TestDeliverDeliveryNoteO2C:
    ...
    async def test_400_wenn_status_nicht_shipped(self):
        ...
        db = _mock_db_for_deliver(_make_dn_row("posted"), True)
    ...
    async def test_auftrag_auf_geliefert_wenn_vollstaendig(self):
        ...
        db = _mock_db_for_deliver(_make_dn_row("shipped"), True)

        match_result = {"vollstaendig_geliefert": True}

        with patch("app.services.sales_match_service.SalesMatchService.match", return_value=match_result):
            try:
                await deliver_delivery_note(ls_id=LS_ID, tenant_id=TENANT, db=db)
            except Exception:
                pass  # dict() on mock row may fail
    ...
    async def test_auftrag_bleibt_wenn_nur_teilgeliefert(self):
        ...
        db = _mock_db_for_deliver(_make_dn_row("shipped"), False)
    ...
    async def test_kein_fehler_wenn_kein_order_id(self):
        ...
        db = _mock_db_for_deliver(_make_dn_row("shipped", sales_order_id=None), False)

        try:
            await deliver_delivery_note(ls_id=LS_ID, tenant_id=TENANT, db=db)
        except Exception:
            pass  # dict() on mock may fail — important: no HTTPException
