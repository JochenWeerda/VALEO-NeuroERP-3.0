...
def _plan() -> tuple[dict[str, Any], str]:
    ...
    feed = client.post(f"{BASE}/feed-catalog/feeds", headers=HEADERS, json={
        "artikel_nummer": f"ACT-{suffix}", "name": f"Ist-Futter {suffix}",
        "art": "Grundfutter", "feed_kind": "forage", "approval_status": "approved",
    })
    assert feed.status_code == 201, feed.text
    feed_id = feed.json()["id"]
    assert client.post(f"{BASE}/feed-catalog/feeds/{feed_id}/products", headers=HEADERS, json={
        "sku": f"ACT-P-{suffix}", "display_name": "Lose Ware", "price_eur_t": "50",
        "freight_eur_t": "5", "packaging_unit": "t", "package_size": 1,
    }).status_code == 201
    assert client.post(f"{BASE}/feed-catalog/feeds/{feed_id}/reference-values", headers=HEADERS, json={
        "nutrient_code": "crude_protein", "value": "80", "unit_code": "g_per_kg",
        "basis": "fresh_matter", "source_type": "analysis", "source_ref": "ACT golden",
    }).status_code == 201
    business = client.post(f"{BASE}/feeding/businesses", headers=HEADERS, json={"name": f"Ist-Betrieb {suffix}"})
    group = client.post(f"{BASE}/lifecycle/groups", headers=HEADERS, json={
        "name": f"Ist-Gruppe {suffix}", "external_ref": f"actual-{suffix}",
        "animal_count": 10, "business_id": business.json()["id"],
    })
    ration = client.post(f"{BASE}/lifecycle/rations", headers=HEADERS, json={
        "group_id": group.json()["id"], "name": f"Ist-Ration {suffix}", "source": "editor",
        "snapshot": {"ration": [{"feed_id": feed_id, "feed_name": feed.json()["name"],
                                  "kg_fm": 10, "mixing_sequence": 1}]},
    })
    version_id = ration.json()["versions"][0]["id"]
    for current, target, reason in (("draft", "in_review", None),
                                    ("in_review", "approved", "Ist-Erfassung geprueft")):
        assert client.post(f"{BASE}/lifecycle/versions/{version_id}/transitions", headers=HEADERS,
          json={"expected_status": current, "target_status": target, "reason": reason}).status_code == 200
    plan = client.post(f"{BASE}/feeding/plans/publish", headers=HEADERS, json={
        "source_ration_version_id": version_id, "animal_count": 10, "dosing_step_kg": "0.1",
        "rounding_mode": "nearest", "valid_from": "2026-07-16", "valid_until": "2026-08-31",
...
def test_actual_feeding_is_plan_bound_idempotent_audited_and_csv_exportable() -> None:
    plan, feed_id = _plan()
    key = f"actual-{uuid4()}"
    payload = {"plan_version_id": plan["id"], "feeding_at": datetime.now(timezone.utc).isoformat(),
      "source": "manual", "source_ref": f"mobile-{uuid4()}", "cause_class": "dosing_error",
      "comment": "Waage wurde nachdosiert", "idempotency_key": key,
    ...
    conflict = client.post(f"{BASE}/feeding/actuals", headers=HEADERS, json={
        **payload, "components": [{"feed_id": feed_id, "actual_kg": "109"}],
    })
    ...
    projected = next(row for row in component_rows.json() if row["actual_record_id"] == record["id"])
    ...
    db = SessionLocal()
    try:
        events = db.execute(text("""SELECT payload FROM public.outbox_events WHERE tenant_id=:tenant_id
          AND event_type='feeding.actual.recorded' AND aggregate_id=:id"""),
          {"tenant_id": TENANT, "id": record["id"]}).scalars().all()
        assert len(events) == 1
        assert json.loads(events[0])["payload"]["plan_version_id"] == plan["id"]
        with pytest.raises(DBAPIError):
            db.execute(text("UPDATE domain_agrar.feeding_actual_records SET comment='x' WHERE id=:id"),
                       {"id": record["id"]})
            db.commit()
    finally:
        db.rollback()
...
def test_actual_feeding_rejects_unknown_component_other_without_comment_and_roles() -> None:
    plan, feed_id = _plan()
    common = {"plan_version_id": plan["id"], "feeding_at": datetime.now(timezone.utc).isoformat(),
      "source": "manual", "source_ref": f"mobile-{uuid4()}", "idempotency_key": f"actual-{uuid4()}"}
    unknown = client.post(f"{BASE}/feeding/actuals", headers=HEADERS, json={**common,
      "cause_class": "normal", "components": [{"feed_id": "not-planned", "actual_kg": 1}]})
    assert unknown.status_code == 409 and "nicht im Plan" in unknown.json()["detail"]
    other = client.post(f"{BASE}/feeding/actuals", headers=HEADERS, json={**common,
      "idempotency_key": f"actual-{uuid4()}", "cause_class": "other", "components": [{"feed_id": feed_id, "actual_kg": 100}]})
    assert other.status_code == 409 and "Kommentar" in other.json()["detail"]

    local = FastAPI()
    local.include_router(router)
    local.dependency_overrides[get_current_user] = lambda: {"sub": "crm", "roles": ["CRM_LESEN"]}
...
def test_actual_feeding_is_tenant_and_business_grant_safe() -> None:
    plan, feed_id = _plan()
    foreign = client.get(f"{BASE}/feeding/actuals", headers={
    ...
    writer_app.dependency_overrides[get_current_user] = lambda: {
        "sub": f"external-writer-{uuid4()}", "roles": ["FUTTERMITTEL_BEARBEITEN"],
    }
    ...
    with TestClient(writer_app, raise_server_exceptions=False) as writer:
        denied = writer.post("/feeding/actuals", json={
            "plan_version_id": plan["id"], "feeding_at": datetime.now(timezone.utc).isoformat(),
            "source": "manual", "source_ref": "denied", "cause_class": "normal",
