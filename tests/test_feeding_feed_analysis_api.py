...
def _feed(suffix: str) -> str:
    response = client.post(
        ...
        headers=HEADERS,
        json={
            "artikel_nummer": f"ANA-{suffix}",
            "name": f"Analysenfutter {suffix}",
            "art": "Grundfutter",
            "feed_kind": "forage",
...
def _analysis(feed_id: str, suffix: str) -> dict:
    response = client.post(
        f"{BASE}/feed-analyses",
        headers=HEADERS,
        json={
            "feed_id": feed_id,
            "bezeichnung": f"Maissilage Probe {suffix}",
            "probe_nr": f"P-{suffix}",
            "labor": "Testlabor",
            "method": "VDLUFA",
            "status": "draft",
...
def test_feed_analysis_full_release_replacement_history_and_tenant_isolation() -> None:
    ...
    validated = client.post(
        f"{BASE}/feed-analyses/{first['id']}/validate",
        headers=HEADERS,
    ...
    released = client.post(
        f"{BASE}/feed-analyses/{first['id']}/transition",
        headers=HEADERS,
        json={
            "target_status": "released",
            "expected_revision": 2,
            "reason": "Laborbefund fachlich freigegeben",
        },
    )
    ...
    try:
        events = (
            db.execute(
                ...
                WHERE tenant_id=:tenant_id AND event_type='feeding.analysis.released'
                  AND aggregate_id=:analysis_id"""
                ),
                {"tenant_id": TENANT, "analysis_id": first["id"]},
            )
            .scalars()
            .all()
        )
        ...
        event = (
            ...
            else __import__("json").loads(events[0])
        )
        assert event["schema_version"] == "1.0"
        assert event["payload"]["feed_id"] == feed_id
    finally:
        db.close()

    second = _analysis(feed_id, f"{suffix}-2")
    second_valid = client.post(
        f"{BASE}/feed-analyses/{second['id']}/validate",
        headers=HEADERS,
        json={"expected_revision": 1},
    )
    assert second_valid.status_code == 200, second_valid.text
    preview = client.post(
        f"{BASE}/feed-analyses/{second['id']}/actions/release",
        headers=HEADERS,
    ...
    assert (
        client.get(f"{BASE}/feed-analyses/{second['id']}", headers=HEADERS).json()[
            "status"
        ]
        == "validated"
    )
    second_release = client.post(
        ...
        json={
            "_mode": "execute",
            "_auditReason": "Aktuellere Probe freigegeben",
        },
    )
    assert second_release.status_code == 200, second_release.text
    assert (
        client.get(f"{BASE}/feed-analyses/{first['id']}", headers=HEADERS).json()[
            "status"
        ]
        == "superseded"
    )
    ...
    history = client.get(
        f"{BASE}/feed-analyses/{second['id']}/history", headers=HEADERS
    )
    ...
    foreign = client.get(
        f"{BASE}/feed-analyses/{second['id']}",
        headers={
...
def test_feed_analysis_enforces_read_write_and_approval_roles() -> None:
    ...
    with TestClient(local, raise_server_exceptions=False) as local_client:
        assert (
            local_client.post("/feed-analyses", json={"bezeichnung": "X"}).status_code
            == 403
        )
        assert (
            local_client.post(
                "/feed-analyses/a/transition",
                json={
                    "target_status": "released",
                    "expected_revision": 1,
                    "reason": "Freigabe Test",
                },
            ).status_code
            == 403
        )
...
def test_import_preview_requires_revisionssicher_document_before_release() -> None:
    ...
    preview = client.post(
        f"{BASE}/feed-analyses/import-preview",
        headers=HEADERS,
    ...
    assert {item["nutrient_code"] for item in parsed["values"]} >= {
        "dry_matter",
        "crude_protein",
    }
    ...
    validated = client.post(
        f"{BASE}/feed-analyses/{analysis_id}/validate",
        headers=HEADERS,
        json={"expected_revision": 1},
    )
    assert validated.status_code == 200, validated.text
    ...
    blocked = client.post(
        f"{BASE}/feed-analyses/{analysis_id}/transition",
        headers=HEADERS,
        json={
            "target_status": "released",
            "expected_revision": 2,
            "reason": "Import fachlich freigeben",
        },
    )
    assert blocked.status_code == 409

    attached = client.post(
        f"{BASE}/feed-analyses/{analysis_id}/document-reference",
        headers=HEADERS,
        json={
            "document_id": f"DMS-{suffix}",
    ...
    released = client.post(
        ...
        json={
            "target_status": "released",
            "expected_revision": 4,
            "reason": "DMS-Beleg und Werte geprueft",
        },
    )
    assert released.status_code == 200, released.text
...
def test_legacy_ground_feed_api_cannot_bypass_revision_and_rejection_history() -> None:
    ...
    created = client.post(
        "/api/v1/agrar/grundfutter-analysen",
        headers=legacy_headers,
        json={
            "bezeichnung": f"Legacy Probe {suffix}",
            "probe_nr": f"LEG-{suffix}",
            "trockensubstanz_os": 35,
            "rohprotein_ts": 15.2,
        },
    )
    assert created.status_code == 201, created.text
    ...
    verified = client.patch(
        f"/api/v1/agrar/grundfutter-analysen/{analysis_id}",
        headers=legacy_headers,
        json={"verifiziert": True, "notizen": "Fachlich geprueft"},
    )
    assert verified.status_code == 200, verified.text
    assert (
        client.get(f"{BASE}/feed-analyses/{analysis_id}", headers=HEADERS).json()[
            "status"
        ]
        == "validated"
    )
    history = client.get(
        f"{BASE}/feed-analyses/{analysis_id}/history", headers=HEADERS
    ).json()
    assert [row["revision"] for row in history] == [2, 1]

    deleted = client.delete(
        f"/api/v1/agrar/grundfutter-analysen/{analysis_id}", headers=legacy_headers
    )
