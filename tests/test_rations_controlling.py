...
def test_daily_observation_is_idempotent_tenant_isolated_and_has_variances() -> None:
    ...
    group = client.post(
        ...
        headers=HEADERS,
        json={
            "external_ref": f"ctrl-{suffix}",
            "name": f"Controlling {suffix}",
            "animal_count": 50,
            "feeding_system": "TMR",
        },
    )
    ...
    ration = client.post(
        ...
        json={
            "group_id": group_id,
            "name": f"Ctrl ration {suffix}",
            "source": "solver",
            "snapshot": {
                "ration": [
                    {
                        "feed_id": "control-feed",
                        "feed_name": "Kontrollfutter",
                        "kg_fm": 20,
                    }
                ],
                "wizard": {"milkYield": 36},
                "optimization_result": {"total_dm_kg": 23, "total_cost_eur_day": 4.8},
            },
        },
    )
    ...
    for current, target in (
        ...
        transition = client.post(
            f"{ROOT}/lifecycle/versions/{version}/transitions",
            headers=HEADERS,
            json={
                "expected_status": current,
                "target_status": target,
                "reason": "Controlling-Test",
            },
        )
        assert transition.status_code == 200, transition.text
    plan = client.post(
        f"{ROOT}/feeding/plans/publish",
        headers=HEADERS,
        json={
            "source_ration_version_id": version,
            "animal_count": 50,
            "dosing_step_kg": "1",
            "rounding_mode": "nearest",
            "valid_from": date.today().isoformat(),
            "reason": "Planversion fuer Controlling-Zeitreihe publizieren",
            "idempotency_key": f"ctrl-plan-{uuid4()}",
        },
    )
    ...
    body = {
        ...
        "source": "manual",
        "source_ref": "stable-day",
        "cow_count": 50,
        "actual_dmi_kg_cow": 22.4,
        "actual_cost_eur_cow": 4.95,
        "actual_milk_kg_cow": 35,
        "milk_price_eur_kg": 0.48,
        "actual_fat_pct": 4.0,
    ...
    second = client.post(
        f"{ROOT}/controlling/observations",
        headers=HEADERS,
        json={**body, "actual_dmi_kg_cow": 22.6},
    )
    assert second.status_code == 201, second.text
    assert second.json()["id"] == first.json()["id"]
    assert second.json()["dmi_deviation_kg"] == -0.4
    assert second.json()["actual_ecm_kg_cow"] == 35.043
    assert second.json()["feeding_plan_version_id"] == plan.json()["id"]
    assert second.json()["iofc_eur_cow"] == 11.85
    series = client.get(
        f"{ROOT}/controlling/series?group_id={group_id}", headers=HEADERS
    )
    assert series.status_code == 200
    assert len(series.json()) == 1
    assert series.json()[0]["plan_version_no"] == plan.json()["version_no"]
    other = client.get(
        f"{ROOT}/controlling/series?group_id={group_id}",
