...
@router.post("/observations", response_model=dict, status_code=201, summary="Taegliche Soll-Ist-Beobachtung erfassen")
async def record_observation(body: DailyObservationIn, db: Session = Depends(get_db), tenant_id: str = Depends(get_tenant_id), user: User = Depends(get_current_user)) -> dict[str, Any]:
    require_roles(user, WRITE_ROLES, detail="Keine Berechtigung zum Erfassen von Fuetterungs-Tageswerten.")
    try: return RationsControllingService(db, tenant_id, str(user.get("sub") or "unknown")).record(body.model_dump())
...
async def get_series(group_id: str | None = None, date_from: date | None = Query(default=None), date_to: date | None = Query(default=None), db: Session = Depends(get_db), tenant_id: str = Depends(get_tenant_id), user: User = Depends(get_current_user)) -> list[dict[str, Any]]:
    require_roles(user, READ_ROLES, detail="Keine Berechtigung zum Lesen der Soll-Ist-Zeitreihe.")
    return RationsControllingService(db, tenant_id, str(user.get("sub") or "unknown")).series(group_id=group_id, date_from=date_from, date_to=date_to)
