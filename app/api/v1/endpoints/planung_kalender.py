...
def _item_out(row: dict[str, Any]) -> CalendarItemOut:
    ...
    return CalendarItemOut(
        ...
        response_status=str(row.get("response_status") or "accepted"),
    )
...
def _layers(value: str | None) -> list[str] | None:
    ...
    if invalid:
        raise HTTPException(
            status_code=422, detail=f"Invalid layers: {', '.join(invalid)}"
        )
    return layers


@router.get(
    "", response_model=list[CalendarItemOut], summary="Planungskalender auflisten"
)
async def list_calendar_items(
    ...
    try:
        user_ref = request.headers.get("X-User-ID") or "default"
        permissions = {
            item.strip()
            for item in request.headers.get("X-Permissions", "").split(",")
            if item.strip()
        }
        team_ids = [
            item.strip() for item in (teams or "").split(",") if item.strip()
        ] or None
        rows = CalendarProjectionService(db).list_team_items(
            tenant_id,
            from_ts,
            to_ts,
            user_ref=user_ref,
            ...
            can_view_details="planung.calendar.team.details" in permissions,
        )
    except PermissionError as exc:
        raise HTTPException(status_code=403, detail=str(exc)) from exc
    except SQLAlchemyError as exc:
        raise HTTPException(
            status_code=503, detail="calendar_items table not available"
        ) from exc
...
@router.post(
    "/reproject",
    response_model=ReprojectOut,
    summary="Planungskalender neu projizieren",
)
async def reproject_calendar(
    ...
    try:
        result = CalendarProjectionService(db).reproject(
            tenant_id, horizon_days=horizon_days
        )
    except SQLAlchemyError as exc:
        db.rollback()
        raise HTTPException(
...
@router.post(
    ...
    response_model=CalendarItemOut,
    summary="Kalender-Vorschlag bestaetigen",
)
async def confirm_calendar_item(
    ...
    row = CalendarProjectionService(db).transition_proposed(
        tenant_id, item_id, "confirmed"
    )
    if not row:
        raise HTTPException(
            status_code=409, detail="Only proposed calendar items can be confirmed"
        )
...
@router.post(
    ...
    response_model=CalendarItemOut,
    summary="Kalender-Vorschlag verwerfen",
)
async def dismiss_calendar_item(
    ...
    row = CalendarProjectionService(db).transition_proposed(
        tenant_id, item_id, "dismissed"
    )
    if not row:
        raise HTTPException(
            status_code=409, detail="Only proposed calendar items can be dismissed"
        )
    return _item_out(row)


@router.get(
    "/ics-token", response_model=IcsTokenOut, summary="ICS Feed Token ausstellen"
)
async def issue_ics_token(
    ...
    return IcsTokenOut(
        token=token, feedUrl=f"/api/v1/planung/kalender/ics?token={token}"
    )


@router.post(
    "/ics-token/rotate", response_model=IcsTokenOut, summary="ICS Feed Token rotieren"
)
...
async def get_ics_feed(
    ...
    now = datetime.now(UTC)
    content = service.ics_content(
        tenant_id, now - timedelta(days=30), now + timedelta(days=366)
    )
