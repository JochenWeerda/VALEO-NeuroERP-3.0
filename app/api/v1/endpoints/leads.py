...
async def get_lead(
    ...
    try:
        lead_repo = container.resolve(LeadRepository)
        lead = await lead_repo.get_by_id(lead_id, "system")  ***REMOVED*** TODO: tenant context
        if not lead:
...
async def update_lead(
    ...
    try:
        lead_repo = container.resolve(LeadRepository)
        lead = await lead_repo.update(lead_id, lead_data.model_dump(exclude_unset=True), "system")  ***REMOVED*** TODO: tenant context
        if not lead:
            raise HTTPException(status_code=404, detail="Lead not found")
        return Lead.model_validate(lead)
    except HTTPException:
...
async def convert_lead(
    ...
    try:
        lead_repo = container.resolve(LeadRepository)
        success = await lead_repo.convert_to_customer(lead_id, customer_id, "system")  ***REMOVED*** TODO: tenant context
        if not success:
...
async def delete_lead(
    ...
    try:
        lead_repo = container.resolve(LeadRepository)
        success = await lead_repo.delete(lead_id, "system")  ***REMOVED*** TODO: tenant context
        if not success:
            raise HTTPException(status_code=404, detail="Lead not found")
    except HTTPException:
        raise
