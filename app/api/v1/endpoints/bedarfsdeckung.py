...
@router.get("/pipeline", response_model=list[TypedObjectOut], summary="Durchdringungs-Pipeline (alle Betriebe nach Chance)")
def pipeline(
...
@router.get("/{kunden_nr}", response_model=TypedObjectOut, summary="Bedarfsdeckungs-Cockpit eines Betriebs")
def cockpit(
    kunden_nr: str,
    db: Session = Depends(get_db),
    tenant_id: str = Depends(get_tenant_id),
