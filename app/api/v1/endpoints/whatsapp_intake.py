...
from app.api.v1.schemas.crm_channel_bundle_schemas import WhatsAppInboxOut
...
@router.post("", response_model=WhatsAppInboxOut, summary="Bestellnachricht erfassen & per AI extrahieren")
...
@router.get("", response_model=list[WhatsAppInboxOut], summary="Posteingang auflisten")
def list_inbox(
...
@router.get("/{inbox_id}", response_model=WhatsAppInboxOut, summary="Eintrag abrufen")
...
@router.post("/{inbox_id}/bestaetigen", response_model=WhatsAppInboxOut, summary="Bestätigen → Kontakt + Beleg-Vorbelegung")
def bestaetigen(
    inbox_id: str,
    body: BestaetigenIn,
    db: Session = Depends(get_db),
...
@router.post("/{inbox_id}/verwerfen", response_model=WhatsAppInboxOut, summary="Verwerfen")
