...
@router.get("/preview", response_model=LeadPreviewOut, summary="Lead-Kandidaten-Vorschau (GAP/LKV, region-universal)")
def lead_preview(
...
@router.get("/leads", response_model=LeadListOut, summary="Übernommene CRM-Leads auflisten")
def list_leads(
...
@router.get("/leads/{lead_id}", response_model=LeadOut, summary="Übernommenen CRM-Lead abrufen")
def get_lead(
    lead_id: str,
    db: Session = Depends(get_db),
    tenant_id: str = Depends(get_tenant_id),
...
@router.get("/leads-count", response_model=LeadCountOut, summary="Anzahl übernommener CRM-Leads")
...
@router.post("/uebernehmen", response_model=LeadUebernahmeOut, summary="Kandidaten als CRM-Leads übernehmen")
def uebernehmen(
...
@router.post("/leads/{lead_id}/convert", response_model=LeadConvertOut, summary="Lead in Kunden konvertieren")
def convert_lead(
    lead_id: str,
    db: Session = Depends(get_db),
    tenant_id: str = Depends(get_tenant_id),
