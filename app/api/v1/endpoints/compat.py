...
@router.get("/portal/feldbuch/schlaege", response_model=list)
async def portal_feldbuch_schlaege(
    tenant_id: Optional[str] = Depends(get_tenant_id),
    db: Session = Depends(get_db),
) -> list[dict[str, Any]]:
    """Felder (Schläge) des Landwirts aus echten DB-Daten."""
    from app.infrastructure.models.agrar_models import FeldbuchSchlag
    try:
        q = db.query(FeldbuchSchlag)
        if tenant_id:
        ...
        return [
            {
                ...
                "name": s.name,
                "flik": s.flik,
                "flaeche": float(s.flaeche or 0),
                "kultur": s.kultur or "",
                "vorkultur": s.vorkultur,
                "gemeinde": s.gemeinde or "",
                "gemarkung": s.gemarkung,
                "bodenart": s.bodenart,
                "ackerzahl": float(s.ackerzahl) if s.ackerzahl else None,
                "status": s.status or "aktiv",
            }
            for s in schlaege
        ]
    except Exception:
...
@router.post("/portal/feldbuch/schlaege", response_model=dict)
async def portal_feldbuch_schlaege_create(
    body: dict = Body(...),
    tenant_id: Optional[str] = Depends(get_tenant_id),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    """Neuen Schlag für den Landwirt anlegen."""
    from app.infrastructure.models.agrar_models import FeldbuchSchlag
    ...
    schlag = FeldbuchSchlag(
        ...
        name=body.get("name", ""),
        flik=body.get("flik"),
        flaeche=float(body.get("flaeche", 0)),
        kultur=body.get("kultur"),
        vorkultur=body.get("vorkultur"),
        gemeinde=body.get("gemeinde"),
        gemarkung=body.get("gemarkung"),
        bodenart=body.get("bodenart"),
        ackerzahl=body.get("ackerzahl"),
        status=body.get("status", "aktiv"),
        created_by=f"portal:{body.get('customer_id', '')}",
    )
    db.add(schlag)
    db.commit()
    db.refresh(schlag)
    return {"id": str(schlag.id), "name": schlag.name, "flaeche": float(schlag.flaeche or 0)}


@router.get("/portal/feldbuch/massnahmen", response_model=list)
async def portal_feldbuch_massnahmen(
    ...
    """Maßnahmen-Journal des Landwirts."""
    from app.infrastructure.models.agrar_models import FeldbuchMassnahme, FeldbuchSchlag
    try:
        q = db.query(FeldbuchMassnahme)
        if tenant_id:
            q = q.filter(FeldbuchMassnahme.tenant_id == tenant_id)
        if schlag_id:
        ...
        if von:
            ...
            q = q.filter(FeldbuchMassnahme.datum >= _date.fromisoformat(von))
        if bis:
            from datetime import date as _date
            q = q.filter(FeldbuchMassnahme.datum <= _date.fromisoformat(bis))
        massnahmen = q.order_by(FeldbuchMassnahme.datum.desc()).all()
        return [
            {
                "id": str(m.id),
                "schlagId": str(m.schlag_id) if m.schlag_id else None,
                "schlagName": m.schlag.name if m.schlag else "",
                "datum": m.datum.isoformat() if m.datum else "",
                "typ": m.typ or "",
                "bezeichnung": m.bezeichnung,
                "mittel": m.mittel,
                "menge": float(m.menge) if m.menge else None,
                "einheit": m.einheit,
                "flaeche": float(m.flaeche) if m.flaeche else None,
                "anwender": m.anwender,
                "quelle": m.quelle or "portal",
                "auflagen": m.auflagen or [],
                "compliant": bool(m.compliant),
                "exportiert": bool(m.exportiert),
...
@router.post("/portal/feldbuch/massnahmen", response_model=dict)
async def portal_feldbuch_massnahmen_create(
    body: dict = Body(...),
    tenant_id: Optional[str] = Depends(get_tenant_id),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    """Neue Maßnahme durch Landwirt erfassen."""
    from app.infrastructure.models.agrar_models import FeldbuchMassnahme
    from datetime import date as _date
    import uuid as _uuid
    massnahme = FeldbuchMassnahme(
        ...
        tenant_id=tenant_id or "",
        customer_id=body.get("customer_id", ""),
        schlag_id=body.get("schlagId") or body.get("schlag_id"),
        datum=_date.fromisoformat(body.get("datum", _date.today().isoformat())),
        typ=body.get("typ", "sonstiges"),
        bezeichnung=body.get("bezeichnung"),
        mittel=body.get("mittel"),
        menge=body.get("menge"),
        einheit=body.get("einheit"),
        flaeche=body.get("flaeche"),
        anwender=body.get("anwender"),
        quelle="portal",
        bemerkung=body.get("bemerkung"),
        compliant=True,
        exportiert=False,
    )
    db.add(massnahme)
    ...
    return {"id": str(massnahme.id), "datum": massnahme.datum.isoformat(), "typ": massnahme.typ}


@router.put("/portal/feldbuch/massnahmen/{massnahme_id}", response_model=dict)
async def portal_feldbuch_massnahmen_update(
    massnahme_id: str,
    body: dict = Body(...),
    tenant_id: Optional[str] = Depends(get_tenant_id),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    """Eigene Maßnahme bearbeiten (nur quelle='portal')."""
    from app.infrastructure.models.agrar_models import FeldbuchMassnahme
    m = db.query(FeldbuchMassnahme).filter(FeldbuchMassnahme.id == massnahme_id).first()
    if not m:
        raise HTTPException(status_code=404, detail="Maßnahme nicht gefunden")
    if m.quelle != "portal":
        raise HTTPException(status_code=403, detail="Nur eigene Portal-Einträge können bearbeitet werden")
    for field in ("bezeichnung", "mittel", "menge", "einheit", "flaeche", "anwender", "bemerkung"):
        if field in body:
            setattr(m, field, body[field])
    if "typ" in body:
        m.typ = body["typ"]
    db.commit()
    return {"id": str(m.id), "updated": True}


@router.get("/portal/feldbuch/stats", response_model=dict)
async def portal_feldbuch_stats(
    tenant_id: Optional[str] = Depends(get_tenant_id),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    """KPIs für das Feldbuch-Dashboard."""
    from app.infrastructure.models.agrar_models import FeldbuchSchlag, FeldbuchMassnahme
    try:
        schlag_q = db.query(FeldbuchSchlag)
        if tenant_id:
            schlag_q = schlag_q.filter(FeldbuchSchlag.tenant_id == tenant_id)
        schlaege = schlag_q.all()
        gesamtflaeche = sum(float(s.flaeche or 0) for s in schlaege)

        massnahme_q = db.query(FeldbuchMassnahme)
        if tenant_id:
            massnahme_q = massnahme_q.filter(FeldbuchMassnahme.tenant_id == tenant_id)
        massnahmen_count = massnahme_q.count()
        valeo_count = massnahme_q.filter(
...
async def portal_feldbuch_export(
    ...
    """Export Feldbuch als CSV (format=csv|ackerschlagkartei)."""
    import csv, io
    ...
    if bis:
        q = q.filter(FeldbuchMassnahme.datum <= _date.fromisoformat(bis))
    massnahmen = q.order_by(FeldbuchMassnahme.datum.asc()).all()

    buf = io.StringIO()
    if format == "ackerschlagkartei":
        writer = csv.writer(buf, delimiter=";")
        writer.writerow([
            "Schlag", "FLIK", "Kultur", "Datum", "Maßnahme-Typ", "Mittel/Produkt",
            "Aufwandmenge", "Einheit", "Fläche_ha", "Anwender",
            "Auflagen", "Wartezeit_Tage", "PSM-Compliant", "Quelle", "Bemerkung"
        ])
        for m in massnahmen:
            writer.writerow([
                m.schlag.name if m.schlag else "",
                m.schlag.flik if m.schlag else "",
                m.schlag.kultur if m.schlag else "",
                m.datum.isoformat() if m.datum else "",
                m.typ or "",
                m.mittel or "",
                m.menge or "",
                m.einheit or "",
                m.flaeche or "",
                m.anwender or "",
                ",".join(m.auflagen) if m.auflagen else "",
                m.wartezeit_tage or "",
                "ja" if m.compliant else "nein",
                m.quelle or "",
                m.bemerkung or "",
            ])
    else:
        writer = csv.DictWriter(buf, fieldnames=[
            "id", "schlag", "datum", "typ", "bezeichnung", "mittel",
            "menge", "einheit", "flaeche", "anwender", "quelle", "bemerkung"
        ], delimiter=";")
        writer.writeheader()
        for m in massnahmen:
            writer.writerow({
                "id": str(m.id),
                "schlag": m.schlag.name if m.schlag else "",
                "datum": m.datum.isoformat() if m.datum else "",
                "typ": m.typ or "",
    ...
    buf.seek(0)
    filename = f"feldbuch_{format}_{_now_iso()[:10]}.csv"
...
@router.post("/portal/feldbuch/import", response_model=dict)
async def portal_feldbuch_import(
    tenant_id: Optional[str] = Depends(get_tenant_id),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    """CSV-Import aus externer Ackerschlagkartei (Stub — erwartet multipart/form-data)."""
    # Stub: Gibt success zurück. Echte Implementation in portal_feldbuch.py
    return {"created": 0, "updated": 0, "errors": ["Import noch nicht implementiert — Backend in Entwicklung"]}
