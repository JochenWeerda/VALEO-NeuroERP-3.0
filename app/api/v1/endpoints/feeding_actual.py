...
def _groups(db: Session, tenant_id: str, user: User, scope: str = "read") -> list[str]:
    if _unrestricted(user):
        return [row["id"] for row in RationLifecycleService(
    ...
    return [row["id"] for row in RationLifecycleService(
        ...
        if scope == "read" or RationLifecycleService(
            db, tenant_id, _actor(user),
        ).has_group_access(row["id"], _actor(user), "write")]


def _translate(exc: Exception) -> HTTPException:
    if isinstance(exc, (FeedingActualNotFound, FeedingPlanNotFound)):
        return HTTPException(status_code=404, detail=str(exc))
    if isinstance(exc, (FeedingActualConflict, ActualFeedingValidationError)):
        return HTTPException(status_code=409, detail=str(exc))
    return HTTPException(status_code=500, detail="Ist-Fuetterung konnte nicht verarbeitet werden.")


@router.post("", response_model=dict[str, Any], status_code=201)
async def record_actual(body: ActualRecordIn, db: Session = Depends(get_db),
                        tenant_id: str = Depends(get_tenant_id),
                        user: User = Depends(get_current_user)) -> dict[str, Any]:
    require_roles(user, WRITE_ROLES)
    try:
        plan = FeedingPlanService(db, tenant_id, _actor(user)).get_version(body.plan_version_id)
...
async def list_actuals(db: Session = Depends(get_db), tenant_id: str = Depends(get_tenant_id),
...
async def list_actual_components(db: Session = Depends(get_db),
                                 tenant_id: str = Depends(get_tenant_id),
                                 user: User = Depends(get_current_user)) -> list[dict[str, Any]]:
    require_roles(user, READ_ROLES)
    return _service(db, tenant_id, user).list_components(group_ids=_groups(db, tenant_id, user))


@router.get("/export.csv")
async def export_actuals_csv(db: Session = Depends(get_db), tenant_id: str = Depends(get_tenant_id),
    ...
    return Response(content=f"\ufeff{content}", media_type="text/csv; charset=utf-8",
