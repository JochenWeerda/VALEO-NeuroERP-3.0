...
async def list_lieferscheine(
    ...
    rows = db.execute(
        text(f"SELECT * FROM einkauf_lieferscheine WHERE {where} ORDER BY lieferschein_datum DESC, created_at DESC LIMIT 500"),  # nosec B608  # reviewed-safe: column names code-controlled, values parameterized
        params,
    ).mappings().all()
...
async def patch_lieferschein(ls_id: str, payload: LieferscheinUpdate, tenant_id: str = Query("system"), db: Session = Depends(get_db)):
    ...
    if values:
        ...
        db.execute(text(f"UPDATE einkauf_lieferscheine SET {set_clause}, updated_at = NOW() WHERE id = :id AND tenant_id = :tenant_id"), params)  # nosec B608  # reviewed-safe: column names code-controlled, values parameterized
        db.commit()
    row = _get_lieferschein_or_404(db, ls_id, tenant_id)
    return Lieferschein(**dict(row), positionen=_list_positions(db, ls_id))
...
async def patch_lieferschein_position(
    ...
    set_clause = ", ".join(f"{k} = :{k}" for k in values.keys())
    db.execute(text(f"UPDATE einkauf_lieferschein_positionen SET {set_clause}, updated_at = NOW() WHERE id = :id"), {"id": pos_id, **values})  # nosec B608  # reviewed-safe: column names code-controlled, values parameterized
    db.commit()
...
async def patch_frachtauftrag(
    ...
    if values:
        set_clause = ", ".join(f"{k} = :{k}" for k in values.keys())
        db.execute(
            text(f"UPDATE einkauf_frachtauftraege SET {set_clause}, updated_at = NOW() WHERE id = :id AND tenant_id = :tenant_id"),  # nosec B608  # reviewed-safe: column names code-controlled, values parameterized
