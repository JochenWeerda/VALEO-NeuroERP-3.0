...
async def list_templates(
    ...
    rows = db.execute(
        text(f"SELECT * FROM domain_crm.crm_campaign_templates {filters} ORDER BY name"),
        params,
    ).fetchall()
...
async def update_template(
    ...
    db.execute(
        text(f"UPDATE domain_crm.crm_campaign_templates SET {set_clauses}, updated_at = :now WHERE id = :id AND tenant_id = :tid"),
        updates,
    )
    db.commit()
    updated = db.execute(
...
async def list_campaigns(
    ...
    rows = db.execute(
        text(f"SELECT * FROM domain_crm.crm_campaigns {filters} ORDER BY created_at DESC"),
        params,
    ).fetchall()
...
async def update_campaign(
    ...
    updates["now"] = datetime.utcnow()
    db.execute(
        text(f"UPDATE domain_crm.crm_campaigns SET {set_clauses}, updated_at = :now WHERE id = :id AND tenant_id = :tid"),
        updates,
    )
    db.commit()
    updated = db.execute(
...
def _transition(campaign_id: str, tenant_id: str, target: str, db: Session) -> dict:
    ...
    db.execute(
        text(f"UPDATE domain_crm.crm_campaigns SET {set_clauses} WHERE id = :id AND tenant_id = :tid"),
        ts_fields,
    )
    db.commit()
    updated = db.execute(
...
async def update_recipient_status(
    ...
    result = db.execute(
        text(f"UPDATE domain_crm.crm_campaign_recipients SET status = :status{extra}{bounce} WHERE campaign_id = :cid AND recipient_id = :rid AND tenant_id = :tid"),
        params,
    )
    if result.rowcount == 0:
