...
def list_partien(
    ...
    try:
        ...
        if status:
            ...
            params["st"] = status
        rows = db.execute(
            text(f"SELECT * FROM domain_agrar.saatgut_partien {where} ORDER BY erstellt_am DESC LIMIT 200"),
            params,
        ).fetchall()
        return [
            SaatgutPartieOut(
...
def update_partie(
    ...
    if updates:
        ...
        try:
            db.execute(
                text(f"UPDATE domain_agrar.saatgut_partien SET {set_clause} WHERE id=:id AND tenant_id=:tid"),
                params,
            )
            db.commit()
        except Exception:
