...
def validate_and_parse_row(
    ...
    dq_datensatz_override: dict[str, Any] | None = None,
) -> tuple[Optional[JournalEntryImportRow], Optional[ImportError]]:
    """Validate and parse a single import row"""
    dq_datensatz = dq_datensatz_override or _build_journal_import_datensatz(row_data)
...
async def import_journal_entries_csv(
    ...
    try:
        ...
        for idx, row_data in enumerate(rows, start=2):  # Start at 2 (header is row 1)
            entry, error = validate_and_parse_row(row_data, idx, dq_context, dq_context[idx - 2])
