...
async def create_customer(
    ...
    try:
        ...
        ***REMOVED*** Check if customer number already exists
...
async def get_customer(
    ...
    try:
        ...
        ***REMOVED*** TODO: Add tenant context from authentication
        customer = await customer_repo.get_by_id(customer_id, "system")  ***REMOVED*** Temporary
        if not customer:
...
async def update_customer(
    ...
    try:
        ...
        ***REMOVED*** Check for customer number conflicts if being updated
        if customer_data.customer_number:
            existing = await customer_repo.get_by_customer_number(customer_data.customer_number, "system")  ***REMOVED*** TODO: tenant context
            if existing and existing.id != customer_id:
                raise HTTPException(status_code=400, detail="Customer number already exists")

        customer = await customer_repo.update(customer_id, customer_data.model_dump(exclude_unset=True), "system")  ***REMOVED*** TODO: tenant context
        if not customer:
            raise HTTPException(status_code=404, detail="Customer not found")
        return Customer.model_validate(customer)
    except HTTPException:
...
async def delete_customer(
    ...
    try:
        customer_repo = container.resolve(CustomerRepository)
        success = await customer_repo.delete(customer_id, "system")  ***REMOVED*** TODO: tenant context
        if not success:
            raise HTTPException(status_code=404, detail="Customer not found")
    except HTTPException:
        raise
