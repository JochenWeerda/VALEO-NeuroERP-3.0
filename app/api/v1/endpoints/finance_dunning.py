...
@router.get("/mahnlauf/candidates", summary="Mahnkandidaten (überfällige Debitoren-OP + nächste Stufe)")
def candidates(
...
@router.get("/mahnlauf/notices", summary="Erzeugte Mahnungen")
def notices(
...
@router.post("/mahnlauf/run", summary="Mahnlauf ausführen (Mahnungen erzeugen + Stufe eskalieren)")
def run(
    body: DunningRunIn,
    db: Session = Depends(get_db),
    tenant_id: str = Depends(get_tenant_id),
