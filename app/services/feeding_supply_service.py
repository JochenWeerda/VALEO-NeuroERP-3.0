...
class FeedingSupplyService:
    ...
    def _match(feed_id: str, catalog: list[dict[str, Any]]) -> dict[str, Any] | None:
        return next(
            (
                ...
                in {
                    str(row["id"]),
                    str(row["artikel_nummer"]),
                    str(row["inventory_article_id"] or ""),
                }
            ),
            None,
        )
    ...
    def project(
        self, *, horizon_days: int, safety_pct: float, subject: str, unrestricted: bool
    ) -> list[dict[str, Any]]:
                ...
                result.append(
                    {
                        ...
                        or (material.get("name") if material else feed_id),
    ...
    def list_handoffs(
        self, *, subject: str, unrestricted: bool
    ) -> list[dict[str, Any]]:
        rows = (
            self.db.execute(
                text("""
    ...
    def create_handoff(
        ...
        *,
        ...
        if len(reason.strip()) < 10:
            raise FeedingSupplyConflict(
                "Uebergabegrund muss mindestens 10 Zeichen enthalten."
            )
        request = {
        ...
        prior = (
            self.db.execute(
                text("""SELECT * FROM domain_agrar.feeding_supply_handoffs
        ...
        if projection["shortage_kg"] is None:
            raise FeedingSupplyConflict(
                "Bestand ist unbekannt; keine Einkaufsmenge ableitbar."
            )
        if projection["shortage_kg"] <= 0:
            raise FeedingSupplyConflict(
                "Keine Unterdeckung fuer eine Einkaufsuebergabe vorhanden."
            )
        if projection["suggested_order_kg"] is None:
            raise FeedingSupplyConflict(
                "Handelseinheit ist unbekannt; Einkaufsmenge muss fachlich geklaert werden."
            )
        handoff_id = str(uuid7())
        row = (
            self.db.execute(
                text("""INSERT INTO domain_agrar.feeding_supply_handoffs
