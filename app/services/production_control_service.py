...
class ProductionControlService:
    ...
    def list_page(self, *, page: int = 1, page_size: int = 50, operation_type: str | None = None,
        ...
        total = self.db.execute(text(f"SELECT COUNT(*) FROM domain_ops.production_operations WHERE {where_sql}"), params).scalar_one()  # nosec B608  # Identifier aus Allowlist/festen Literalen; Werte nur gebunden (:params)
