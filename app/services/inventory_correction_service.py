...
def storno_korrektur(
    ...
    if (orig.get("reference_type") or "").upper() == "STORNO":
    ...
    existing_storno = db.execute(
        text(
            ...
            "WHERE storno_ref = :ref AND reference_type = 'STORNO' AND tenant_id = :tid "
            "LIMIT 1"
        ),
        ...
        text("""
            INSERT INTO domain_inventory.inventory_stock_movements
                (id, tenant_id, article_id, warehouse_id, movement_type,
                 quantity, unit, reference_type, storno_ref, notes)
            VALUES (:id, :tenant_id, :article_id, :warehouse_id, :movement_type,
                    :quantity, :unit, 'STORNO', :storno_ref, :notes)
        """),
        {
            "id": storno_id,
            "tenant_id": tenant_id,
