...
def transition_budget_status(
    ...
    db.execute(
        text(f"UPDATE domain_controlling.controlling_budgets SET {set_clauses} WHERE id = :id"),  # nosec S608 — set_clauses is assembled only from fixed code-controlled fragments; values are parameterized.
        params,
    )
