...
class RationLifecycleService:
    ...
    def create_group(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        validate_group_parameters(
            ...
            gestation_day=payload.get("gestation_day"),
            milk_fat_pct=payload.get("milk_fat_pct"),
            milk_protein_pct=payload.get("milk_protein_pct"),
        ...
        try:
            ...
            raise RationLifecycleConflict(
                "Eine Fuetterungsgruppe mit dieser externen Referenz existiert bereits."
            ) from exc

    def list_groups(
        self, *, active_only: bool = True, subject: str = "", unrestricted: bool = False
    ) -> list[dict[str, Any]]:
        rows = (
            self.db.execute(
                text("""
    ...
    def _validate_group_parent(
        self, business_id: str | None, herd_id: str | None
    ) -> None:
        if herd_id and not business_id:
            raise RationLifecycleConflict(
                "Eine Herde erfordert einen Fuetterungsbetrieb."
            )
        if business_id:
            ...
            if not business:
                raise RationLifecycleNotFound(
                    "Aktiver Fuetterungsbetrieb nicht gefunden."
                )
        if herd_id:
            herd = self.db.execute(
                text("""
            ...
            if not herd:
                raise RationLifecycleNotFound(
                    "Aktive Herde im Fuetterungsbetrieb nicht gefunden."
                )
    ...
    def update_group(self, group_id: str, patch: dict[str, Any]) -> dict[str, Any]:
        ...
        if unknown:
            raise RationLifecycleConflict(
                f"Unbekannte Gruppenfelder: {', '.join(sorted(unknown))}"
            )
        ...
        validate_group_parameters(
            profile=values["profile_code"],
            pregnancy_status=values["pregnancy_status"],
            gestation_day=values.get("gestation_day"),
            milk_fat_pct=values.get("milk_fat_pct"),
            milk_protein_pct=values.get("milk_protein_pct"),
        ...
        if not row:
            raise RationLifecycleConflict(
                "Revision der Fuetterungsgruppe ist nicht mehr aktuell."
            )
        result = _dict(row)
    ...
    def create_version(
        ...
        *,
        ...
        try:
            ...
            self.db.rollback()
            raise RationLifecycleConflict(
                "Dieser Rationsinhalt ist bereits als Version gespeichert."
            ) from exc
    ...
    def _create_version_locked(
        ...
        *,
        ...
        """),
                {"tenant_id": self.tenant_id, "ration_id": ration_id},
            )
            .mappings()
            .first()
        )
        ...
        if based_on_version_id:
            ...
            """),
                {
                    "tenant_id": self.tenant_id,
                    "group_id": ration["group_id"],
                    "version_id": based_on_version_id,
                },
            ).first()
            if not base_exists:
                raise RationLifecycleNotFound(
                    "Basisversion in dieser Fuetterungsgruppe nicht gefunden."
                )
        ...
        self.db.execute(
            text("UPDATE domain_agrar.rations SET updated_at=now() WHERE id=:id"),
            {"id": ration_id},
        )
        result = _dict(row)
    ...
    def get_ration(
        ...
        *,
        ...
        if result["versions"]:
            ...
            readiness = (
                latest.get("snapshot", {}).get("readiness", {})
                if isinstance(latest.get("snapshot"), dict)
                else {}
            )
            result["latest_readiness_status"] = readiness.get("status", "not_checked")
            result["latest_readiness_blockers"] = int(
                readiness.get("blocker_count", 0) or 0
            )
            result["latest_readiness_warnings"] = int(
    ...
    def transition(
        ...
        *,
        ...
        validate_transition(
            row["status"], target, reason=reason, feeding_start=effective_start
        )
        readiness = (
            row["snapshot"].get("readiness", {})
        ...
        if (
            ...
            and blockers > 0
            and not (reason or "").startswith("OVERRIDE:")
        ):
            raise RationLifecycleConflict(
        ...
        if target is RationStatus.ACTIVE:
            ...
            for old in superseded:
                ...
                self._audit(
                    ration_id=old["ration_id"],
                    version_id=old["version_id"],
                    event_type="superseded",
                    from_status="active",
                    to_status="retired",
                    reason=f"Durch Version {version_id} ersetzt.",
                    delta={"replacement_version_id": version_id},
                )
        updated = (
            self.db.execute(
                text("""
        ...
        self._audit(
            ...
            delta={
                ...
                "feeding_start": effective_start.isoformat()
                if effective_start
                else None,
        ...
        if target is RationStatus.ACTIVE:
            emit_feeding_event(
                self.db,
                tenant_id=self.tenant_id,
                event_type="feeding.ration.version.activated",
                ...
                payload={
                    ...
                    "superseded_version_ids": [
                        item["version_id"] for item in superseded
                    ],
                    "actor": self.actor,
                },
            )
