...
class FeedingFeedAnalysisService:
    ...
    def _head(self, analysis_id: str, *, lock: bool = False) -> dict[str, Any]:
        ...
        """ + suffix), {"tenant_id": self.tenant_id, "analysis_id": analysis_id}).mappings().first()
        if not row:
    ...
    def _assert_feed(self, feed_id: str | None) -> None:
        ...
        """), {"tenant_id": self.tenant_id, "feed_id": feed_id}).scalar()
        if not exists:
            raise FeedAnalysisConflict("Zugeordnetes Futtermittel wurde nicht gefunden.")

    def list_analyses(self, *, status: str | None = None, feed_id: str | None = None,
        ...
        """), {"tenant_id": self.tenant_id, "status": status, "feed_id": feed_id,
    ...
    def values(self, analysis_id: str) -> list[dict[str, Any]]:
        ...
        """), {"tenant_id": self.tenant_id, "analysis_id": analysis_id}).mappings().all()
    ...
    def create_analysis(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if status not in {AnalysisStatus.UPLOADED, AnalysisStatus.MAPPED, AnalysisStatus.DRAFT}:
            raise FeedAnalysisConflict("Neue Analysen duerfen nur uploaded, mapped oder draft sein.")
        ...
        try:
            ...
            """), {
                "id": analysis_id, "tenant_id": self.tenant_id, "actor": self.actor,
                "feed_id": payload.get("feed_id"), "bezeichnung": payload["bezeichnung"],
                "scope_code": payload.get("scope_code", "default"),
                "probe_nr": payload.get("probe_nr"), "probenart": payload.get("probenart"),
                "labor": payload.get("labor"), "analyse_datum": payload.get("analyse_datum"),
                "method": payload.get("method"), "sampled_at": payload.get("sampled_at"),
                "valid_from": valid_from, "valid_until": payload.get("valid_until"),
                "original_document_id": payload.get("original_document_id"),
                "original_sha256": payload.get("original_sha256"), "quelle_datei": payload.get("quelle_datei"),
                "status": status.value, "notizen": payload.get("notizen"),
            })
    ...
    def record_legacy_create(self, analysis_id: str) -> dict[str, Any]:
        ...
        """), {"tenant_id": self.tenant_id, "analysis_id": analysis_id,
                "revision": int(head["revision"])}).scalar()
        if not existing:
            self._append_revision(analysis_id, int(head["revision"]), "Anlage ueber kompatible Grundfutteranalyse-API")
        self.db.commit()
        return self.get_analysis(analysis_id)

    def record_legacy_update(self, analysis_id: str, *, verified: bool, reason: str) -> dict[str, Any]:
        head = self._head(analysis_id, lock=True)
        if head["status"] in {AnalysisStatus.RELEASED.value, AnalysisStatus.SUPERSEDED.value,
        ...
        self.db.execute(text("""
          ...
          WHERE tenant_id=:tenant_id AND id=:analysis_id
        """), {"status": target_status, "revision": revision, "actor": self.actor,
                "tenant_id": self.tenant_id, "analysis_id": analysis_id})
    ...
    def add_value(self, analysis_id: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            self.db.execute(text("""
              ...
              WHERE tenant_id=:tenant_id AND id=:analysis_id
            """), {"revision": revision, "actor": self.actor, "tenant_id": self.tenant_id,
                    "analysis_id": analysis_id})
            self._append_revision(analysis_id, revision, f"Analysewert {row['nutrient_code']} erfasst")
            self.db.commit()
            return row
        except Exception:
            self.db.rollback()
    ...
    def _add_value(self, analysis_id: str, payload: dict[str, Any]) -> dict[str, Any]:
        normalized = normalize_analysis_value(
            payload["nutrient_code"], payload["original_value"], payload["original_unit_code"],
            payload["canonical_unit_code"], payload["basis"], payload.get("value_status", "measured"),
        )
        revision = int(self.db.execute(text("""
        ...
        """), {"tenant_id": self.tenant_id, "analysis_id": analysis_id,
                "nutrient_code": normalized.nutrient_code}).scalar_one())
        value_id = str(payload.get("id") or uuid7())
        params = {
            "id": value_id, "tenant_id": self.tenant_id, "analysis_id": analysis_id,
            "nutrient_code": normalized.nutrient_code, "original_value": normalized.original_value,
            "original_unit_code": normalized.original_unit_code, "canonical_value": normalized.canonical_value,
            "canonical_unit_code": normalized.canonical_unit_code, "basis": normalized.basis,
            "value_status": normalized.value_status.value, "method": payload.get("method"),
            "detection_limit": payload.get("detection_limit"), "confidence": payload.get("confidence"),
            "source_ref": payload.get("source_ref"), "revision": revision, "actor": self.actor,
        }
        self.db.execute(text("""
    ...
    def validate(self, analysis_id: str, expected_revision: int) -> dict[str, Any]:
        ...
        if int(head["revision"]) != expected_revision:
            raise FeedAnalysisConflict(f"Versionskonflikt: erwartet {expected_revision}, aktuell {head['revision']}.")
        if head["status"] not in {AnalysisStatus.DRAFT.value, AnalysisStatus.VALIDATED.value}:
        ...
        if head.get("quelle_datei") and not head.get("original_document_id"):
            findings.append({
                "code": "original-document-not-archived", "severity": "blocker",
                "message": "Importierter Originalbeleg ist noch nicht revisionssicher im DMS referenziert.",
            })
        try:
            self.db.execute(text("""DELETE FROM domain_shared.feeding_feed_analysis_findings
              WHERE tenant_id=:tenant_id AND analysis_id=:analysis_id"""),
              {"tenant_id": self.tenant_id, "analysis_id": analysis_id})
            for finding in findings:
                ...
                """), {"id": str(uuid7()), "tenant_id": self.tenant_id, "analysis_id": analysis_id,
            ...
            self.db.execute(text("""UPDATE domain_shared.grundfutter_analysen
    ...
    def transition(self, analysis_id: str, target: AnalysisStatus | str, expected_revision: int,
                   reason: str) -> dict[str, Any]:
        head = self._head(analysis_id, lock=True)
        if int(head["revision"]) != expected_revision:
            raise FeedAnalysisConflict(f"Versionskonflikt: erwartet {expected_revision}, aktuell {head['revision']}.")
        try:
            target_status = transition_analysis(head["status"], target, self.findings(analysis_id))
        except ValueError as exc:
            raise FeedAnalysisConflict(str(exc)) from exc
        if target_status == AnalysisStatus.RELEASED and not head.get("feed_id"):
            raise FeedAnalysisConflict("Freigabe erfordert die Zuordnung zu einem Futtermittel.")
        revision = expected_revision + 1
        try:
            if target_status == AnalysisStatus.RELEASED:
                replaced = self.db.execute(text("""
                ...
                """), {"tenant_id": self.tenant_id, "feed_id": head["feed_id"],
                ...
                """), {"actor": self.actor, "tenant_id": self.tenant_id, "feed_id": head["feed_id"],
                        "scope_code": head["scope_code"],
                        "analysis_id": analysis_id})
                for prior in replaced:
                    self._append_revision(str(prior["id"]), int(prior["revision"]) + 1,
            ...
            self.db.execute(text("""
              ...
              WHERE tenant_id=:tenant_id AND id=:analysis_id
            """), {"status": target_status.value, "active": target_status == AnalysisStatus.RELEASED,
                    "revision": revision, "actor": self.actor, "tenant_id": self.tenant_id,
                    "analysis_id": analysis_id})
            self._append_revision(analysis_id, revision, reason)
            self.db.commit()
    ...
    def attach_document(self, analysis_id: str, document_id: str, sha256: str,
        ...
        self.db.execute(text("""UPDATE domain_shared.grundfutter_analysen
          ...
          {"document_id": document_id, "digest": digest, "revision": revision, "actor": self.actor,
           "tenant_id": self.tenant_id, "analysis_id": analysis_id})
    ...
    def _append_revision(self, analysis_id: str, revision: int, reason: str) -> None:
        head = self._head(analysis_id)
        snapshot = {key: value for key, value in head.items() if key not in {"updated_at", "created_at"}}
        self.db.execute(text("""
        ...
        """), {"id": str(uuid7()), "tenant_id": self.tenant_id, "analysis_id": analysis_id,
                "revision": revision, "snapshot": json.dumps(snapshot, default=str, ensure_ascii=False),
