...
class FeedingActualService:
    ...
    def _plan(self, version_id: str) -> dict[str, Any]:
        try:
            plan = FeedingPlanService(self.db, self.tenant_id, self.actor).get_version(
                version_id
            )
        except FeedingPlanNotFound as exc:
            raise FeedingActualNotFound(str(exc)) from exc
        if plan["plan_status"] != "current":
            raise FeedingActualConflict(
                "Ist-Fuetterung kann nur gegen eine aktuelle Planversion erfasst werden."
            )
        return plan
    ...
    def _feed_context(self, feed_id: str, feeding_at: datetime) -> dict[str, Any]:
        material = (
          ...
          WHERE ef.tenant_id=:tenant_id AND :feed_id IN
            (ef.id,ef.artikel_nummer,COALESCE(ef.inventory_article_id,'')) LIMIT 1"""),
                {"tenant_id": self.tenant_id, "feed_id": feed_id, "day": feeding_at},
            )
            .mappings()
            .first()
        )
        ...
        nutrients = (
            self.db.execute(
                text("""SELECT DISTINCT ON (nutrient_code)
        ...
        return {
            "price_eur_t": material["price_eur_t"],
    ...
    def record(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            payload["cause_class"] == "other"
            and len((payload.get("comment") or "").strip()) < 10
        ):
            raise ActualFeedingValidationError(
                "Ursache 'other' erfordert einen Kommentar mit mindestens 10 Zeichen."
            )
        ...
        canonical = {
            **payload,
            "feeding_at": feeding_at.isoformat(),
        ...
        prior = (
            self.db.execute(
                text("""SELECT id,request_hash FROM domain_agrar.feeding_actual_records
        ...
        if payload.get("supersedes_id"):
            ...
            if predecessor["plan_version_id"] != plan["id"]:
                raise FeedingActualConflict(
                    "Korrektur und Vorgaenger muessen dieselbe Planversion referenzieren."
                )

        record_id = str(uuid7())
        self.db.execute(
            text("""INSERT INTO domain_agrar.feeding_actual_records
           ...
           :cause_class,:comment,CAST(:context_json AS jsonb),:supersedes_id,:idempotency_key,:request_hash,:actor)"""),
            {
                **payload,
                ...
                "feeding_at": feeding_at,
                "context_json": json.dumps(
                    payload.get("context") or {}, default=str, ensure_ascii=False
                ),
        ...
        for component in payload["components"]:
            ...
            if instruction["target_batch_kg"] is None:
                raise FeedingActualConflict(
                    f"Planmenge fuer {component['feed_id']} ist unbekannt."
                )
            variance = calculate_component_actual(
                target_kg=instruction["target_batch_kg"],
                actual_kg=component["actual_kg"],
            )
            context = self._feed_context(str(component["feed_id"]), feeding_at)
            consequence = calculate_value_consequences(
                target_kg=variance.target_kg,
                actual_kg=variance.actual_kg,
                price_eur_t=context["price_eur_t"],
                nutrient_values=context["nutrients"],
            )
            consequence["source_values"] = context["nutrients"]
            self.db.execute(
                text("""INSERT INTO domain_agrar.feeding_actual_components
               ...
               :actual,:delta,:pct,CAST(:consequence AS jsonb))"""),
                {
                    ...
                    "feed_name": instruction.get("feed_name"),
                    "target": variance.target_kg,
                    "actual": variance.actual_kg,
                    "delta": variance.delta_kg,
                    "pct": variance.delta_pct,
                    "consequence": json.dumps(
                        consequence, default=str, ensure_ascii=False
                    ),
                },
            )
        emit_feeding_event(
            self.db,
            tenant_id=self.tenant_id,
            event_type="feeding.actual.recorded",
    ...
    def get(self, record_id: str) -> dict[str, Any]:
        row = (
            self.db.execute(
                text("""SELECT r.*,g.name AS group_name,pv.version_no AS plan_version_no
          ...
          WHERE r.tenant_id=:tenant_id AND r.id=:id"""),
                {"tenant_id": self.tenant_id, "id": record_id},
            )
            .mappings()
            .first()
        )
    ...
    def list(self, *, group_ids: list[str]) -> list[dict[str, Any]]:
        ...
        ids = (
            self.db.execute(
                text("""SELECT id FROM domain_agrar.feeding_actual_records
        ...
        for record in self.list(group_ids=group_ids):
            for component in record["components"]:
                ...
                result.append(
                    {
                        "id": component["id"],
                        "actual_record_id": record["id"],
                        "plan_version_id": record["plan_version_id"],
                        "plan_version_no": record["plan_version_no"],
                        "group_id": record["group_id"],
                        "group_name": record["group_name"],
                        "feeding_at": record["feeding_at"],
                        "cause_class": record["cause_class"],
                        "comment": record["comment"],
                        "source": record["source"],
                        "feed_id": component["feed_id"],
                        "feed_name": component["feed_name"],
                        "target_kg": component["target_kg"],
                        "actual_kg": component["actual_kg"],
                        "delta_kg": component["delta_kg"],
                        "delta_pct": component["delta_pct"],
                        "cost_delta_eur": (consequence.get("cost") or {}).get(
                            "delta_eur"
                        ),
                        "nutrient_delta_summary": ", ".join(
                            f"{item['code']}: {item['delta']} {item['result_unit']}"
                            for item in nutrients
                        )
                        or None,
                        "missing_value_summary": ", ".join(
                            consequence.get("missing") or []
                        )
                        or None,
                    }
                )
