...
class FeedingPlanService:
    ...
    async def publish(
        ...
        *,
        ...
        if len(reason.strip()) < 10:
            raise FeedingPlanConflict(
                "Publikationsgrund muss mindestens 10 Zeichen enthalten."
            )
        request = {
            "source": source_ration_version_id,
            "animal_count": animal_count,
            "dosing_step_kg": str(dosing_step_kg),
        ...
        prior = (
            self.db.execute(
                text(
                    "SELECT id,request_hash FROM domain_agrar.feeding_plan_versions WHERE tenant_id=:tenant_id AND idempotency_key=:key"
                ),
                {"tenant_id": self.tenant_id, "key": idempotency_key},
            )
            .mappings()
            .first()
        )
        ...
        source = (
            self.db.execute(
                text("""SELECT rv.id,rv.snapshot,rv.ration_id,r.group_id,r.name,lc.status
          ...
          WHERE rv.tenant_id=:tenant_id AND rv.id=:id FOR UPDATE OF rv"""),
                {"tenant_id": self.tenant_id, "id": source_ration_version_id},
            )
            .mappings()
            .first()
        )
        ...
        if source["status"] not in {"approved", "active"}:
            raise FeedingPlanConflict(
                "Nur approved oder active Rationsversionen duerfen publiziert werden."
            )
        self.db.execute(
        ...
        instructions = build_mixing_instructions(
            dict(source["snapshot"]),
            animal_count=animal_count,
            dosing_step_kg=dosing_step_kg,
            rounding_mode=rounding_mode,
        )
        plan = (
            self.db.execute(
                text("""INSERT INTO domain_agrar.feeding_plans (id,tenant_id,group_id,name,created_by)
        ...
        latest = int(
            self.db.execute(
                text(
                    "SELECT COALESCE(MAX(version_no),0) FROM domain_agrar.feeding_plan_versions WHERE tenant_id=:tenant_id AND plan_id=:plan_id"
                ),
                {"tenant_id": self.tenant_id, "plan_id": plan["id"]},
            ).scalar_one()
        )
        version_id = str(uuid7())
        self.db.execute(
            text("""INSERT INTO domain_agrar.feeding_plan_versions
        ...
        for item in instructions:
            self.db.execute(
                text("""INSERT INTO domain_agrar.feeding_mixing_instructions
              ...
              VALUES (:id,:tenant_id,:version_id,:sequence,:feed_id,:feed_name,:kg_fm,:raw,:target,:delta)"""),
                {
                    ...
                    "kg_fm": item.kg_fm_per_animal,
                    "raw": item.raw_batch_kg,
                    "target": item.target_batch_kg,
                    "delta": item.rounding_delta_kg,
                },
            )
        emit_feeding_event(
            self.db,
            tenant_id=self.tenant_id,
            event_type="feeding.plan.published",
            aggregate_id=plan["id"],
            payload={
                "plan_id": plan["id"],
                "plan_version_id": version_id,
                "source_ration_version_id": source_ration_version_id,
                "group_id": source["group_id"],
                "version_no": latest + 1,
                "valid_from": str(valid_from),
                "valid_until": str(valid_until),
    ...
    def get_version(self, version_id: str) -> dict[str, Any]:
        row = (
            self.db.execute(
                text("""SELECT pv.*,p.group_id,p.name,g.name AS group_name,
          ...
          WHERE pv.tenant_id=:tenant_id AND pv.id=:id"""),
                {"tenant_id": self.tenant_id, "id": version_id},
            )
            .mappings()
            .first()
        )
        ...
        result["instructions"] = [
            ...
            for x in self.db.execute(
                text(
                    "SELECT * FROM domain_agrar.feeding_mixing_instructions WHERE tenant_id=:tenant_id AND plan_version_id=:id ORDER BY sequence"
                ),
                {"tenant_id": self.tenant_id, "id": version_id},
            )
            .mappings()
    ...
    def list_current(
        self, *, subject: str = "", unrestricted: bool = False
    ) -> list[dict[str, Any]]:
        current = [
            row
            for row in self.list_versions(subject=subject, unrestricted=unrestricted)
