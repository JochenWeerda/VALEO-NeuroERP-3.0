...
class CustomJsonFormatter(jsonlogger.JsonFormatter):
    ...
    def add_fields(self, log_record: Dict[str, Any], record: logging.LogRecord, message_dict: Dict[str, Any]) -> None:
        ...
        ***REMOVED*** Add custom fields
        log_record['service'] = 'valeo-neuro-erp'
        log_record['version'] = '3.0.0'

        ***REMOVED*** Add request ID if available (will be set by middleware)
...
def setup_logging():
    ...
    ***REMOVED*** Create console handler
    ...
    if settings.LOG_FORMAT == "json":
        ***REMOVED*** JSON format for production
        formatter = CustomJsonFormatter(
            fmt='%(asctime)s %(name)s %(levelname)s %(message)s',
            datefmt='%Y-%m-%d %H:%M:%S'
        )
    else:
        ***REMOVED*** Human-readable format for development
    ...
    ***REMOVED*** Set specific log levels for noisy libraries
...
def get_request_logger(request_id: str = None):
    ...
    if request_id:
        ***REMOVED*** Create a logger adapter with request ID
