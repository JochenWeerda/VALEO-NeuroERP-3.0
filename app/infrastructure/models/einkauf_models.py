...
class EinkaufBestellung(Base):
    ...
    # Fuehrende Maske: L3-Kopf (WWS_BESTELLUNG1) plus drei VALEO-Bestellfaelle.
    bestellfall      = Column(String(32), default="bestand_abgleich")
                       # bestand_abgleich | direktlieferung | innovation
    ansprechpartner  = Column(String(200))
    kreditor_konto   = Column(String(50))
    lieferant_nr     = Column(String(50))
    kostenstelle     = Column(String(50))
    kommission       = Column(String(100))
    ladetermin       = Column(Date)
    ladetermin_ab    = Column(Date)
    lade_datum       = Column(Date)
    incoterms        = Column(String(10))
    lieferadresse    = Column(Text)
    zahlungsbedingung = Column(String(80))
    skonto1_tage     = Column(Integer)
    skonto1_prozent  = Column(Numeric(5, 2))
    skonto2_tage     = Column(Integer)
    skonto2_prozent  = Column(Numeric(5, 2))
    netto_tage       = Column(Integer)
    fremdwaehrung    = Column(String(3))
    umrechnungsfaktor = Column(Numeric(12, 6))
    anfrage_nr       = Column(String(50))
    angebot_nr       = Column(String(50))
    auftrag_nr       = Column(String(50))
    abverkauf_horizont = Column(String(20))
                       # taeglich | woechentlich | monatlich | saisonal
    bedarfsmenge     = Column(Numeric(14, 3))
    mindestbestellmenge = Column(Numeric(14, 3))
    maximalbestellmenge = Column(Numeric(14, 3))
    artikelgruppe    = Column(String(80))
    lagerplatz_opt   = Column(Boolean, default=False)
    fracht_opt       = Column(Boolean, default=False)
    opportunitaetskostensatz = Column(Numeric(14, 4))
    palettenstellplatz_kosten = Column(Numeric(14, 4))
    lagerkosten_satz = Column(Numeric(14, 4))
    verkaufsbeleg_id = Column(String(64))
    kunden_id        = Column(String(64))
    direktlieferung  = Column(Boolean, default=False)
    ueberschlag_lager = Column(Boolean, default=False)
    neuer_artikel    = Column(Boolean, default=False)
    innovationshinweis = Column(Text)
...
class EinkaufBestellungPosition(Base):
    ...
    notiz            = Column(Text)
    gebinde_menge    = Column(Numeric(14, 3))
    gebinde_einheit  = Column(String(20))
    gebinde_schluessel = Column(String(20))
    gewicht_kg       = Column(Numeric(14, 3))
    kontrakt_nr      = Column(String(50))
    lagerhalle       = Column(String(50))
    lagerfach        = Column(String(50))
    mindestmenge     = Column(Numeric(14, 3))
    maximalmenge     = Column(Numeric(14, 3))
