...
class DailyReportService:
    ...
    def _generate_rep_report(self, sales_rep_id: str, report_date: datetime) -> Optional[Dict[str, Any]]:
        ...
        ***REMOVED*** Get visit reports for the day
    ...
    def _calculate_summary(self, activities: List[Activity], visits: List[VisitReport]) -> Dict[str, Any]:
        ...
        ***REMOVED*** Calculate total time spent
        ...
        ***REMOVED*** Count orders and quotes
    ...
    def format_report_markdown(self, sales_rep_data: Dict[str, Any]) -> str:
        ...
        ***REMOVED*** Build markdown report
        lines = []

        ***REMOVED*** Header
        lines.append(f"***REMOVED*** Tagesprotokoll – Außendienst ({rep.first_name} {rep.last_name})")
        lines.append(f"**Datum:** {report_date.strftime('%d.%m.%Y')}")
        lines.append("")

        ***REMOVED*** Summary
        lines.append("***REMOVED******REMOVED*** Zusammenfassung")
        ...
        ***REMOVED*** Route
        if route:
            lines.append("***REMOVED******REMOVED*** Route")
            for stop in route:
        ...
        ***REMOVED*** Main topics
        if summary['main_topics']:
            lines.append("***REMOVED******REMOVED*** Hauptthemen")
            for topic in summary['main_topics'][:5]:  ***REMOVED*** Top 5
                lines.append(f"- {topic}")
            lines.append("")

        ***REMOVED*** Products discussed
        if summary['products_discussed']:
            lines.append("***REMOVED******REMOVED*** Besprochene Produkte")
            for product in summary['products_discussed'][:5]:  ***REMOVED*** Top 5
                lines.append(f"- {product}")
            lines.append("")

        ***REMOVED*** Activities
        if activities:
            lines.append("***REMOVED******REMOVED*** Aktivitäten")
            for activity in activities:
                lines.append(f"***REMOVED******REMOVED******REMOVED*** {activity['time']} - {activity['type'].title()}")
        ...
        ***REMOVED*** Visits
        if visits:
            lines.append("***REMOVED******REMOVED*** Besuchsberichte")
            for visit in visits:
                lines.append(f"***REMOVED******REMOVED******REMOVED*** {visit['time']} - {visit['customer']}")
                if visit['contact']:
        ...
        ***REMOVED*** Follow-ups
        if follow_ups:
            lines.append("***REMOVED******REMOVED*** Geplante Nachverfolgungen")
            for follow_up in follow_ups[:10]:  ***REMOVED*** Top 10
    ...
    def send_daily_reports(self, reports: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for rep_id, report_data in reports.items():
            try:
                ...
                ***REMOVED*** Here you would integrate with your notification/email service
                ***REMOVED*** For now, we'll just log the report
                logger.info(f"Daily report for {report_data['sales_rep'].username}:")
                logger.info(markdown_report)

                ***REMOVED*** TODO: Send via email, Teams, Telegram, etc.
                ***REMOVED*** self.email_service.send_report(report_data['sales_rep'].email, markdown_report)
                ***REMOVED*** self.notification_service.send_to_management(markdown_report)
