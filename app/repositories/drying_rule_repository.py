"""SQLAlchemy-backed implementation of DryingRuleRepository for the drying rule engine."""
...
class DbDryingRuleRepository(_DryingRuleRepository):
    ...
    def find_rule_set(self, *, crop_code: str, site_id: Optional[str], calc_date, contract_id: Optional[str] = None, customer_id: Optional[str] = None) -> _DryingRuleSet:
        ...
        if not row:
            raise ValueError(f"No drying rule set found for crop_code={crop_code} (site_id={site_id}, contract_id={contract_id}, customer_id={customer_id})")
        return self._row_to_rule_set(row)
