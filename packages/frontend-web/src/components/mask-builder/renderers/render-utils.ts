import type { ScreenLayoutMode } from '../schema'
...
export function layoutClasses(mode: ScreenLayoutMode | undefined): {
  root: string
  header: string
  fields: string
  touchTarget: string
  ...
  return {
    root: 'space-y-6 p-4 md:p-8',
    header: 'flex flex-col gap-4 rounded-md border border-border bg-card p-6 shadow-sm md:flex-row md:items-center md:justify-between',
    fields: 'grid gap-4 md:grid-cols-2 xl:grid-cols-3',
    touchTarget: '',
  }
}
