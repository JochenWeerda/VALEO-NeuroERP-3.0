...
export default function RahmenvertraegePage(): JSX.Element {
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      {auslaufend > 0 && <Card className="border-orange-500 bg-orange-50"><CardContent className="pt-4"><div className="flex items-center gap-2 text-orange-900"><AlertTriangle className="h-5 w-5" /><span className="font-semibold">{auslaufend} Vertrag/Vertraege laufen bald aus!</span></div></CardContent></Card>}
      <div className="grid gap-4 md:grid-cols-3">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Vertraege Gesamt</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><FileText className="h-5 w-5 text-blue-600" /><span className="text-2xl font-bold">{vertraege.length}</span></div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Aktiv</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold text-green-600">{vertraege.filter((v) => v.status === 'aktiv').length}</span></CardContent></Card>
      ...
      <Card><CardContent className="pt-6"><DataTable data={vertraege} columns={columns} /></CardContent></Card>
    </div>
  )
}
