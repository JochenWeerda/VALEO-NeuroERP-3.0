...
const mockVerladungen: VerladungItem[] = [
...
export default function VerladungenListePage(): JSX.Element {
  ...
  const { data: verladungen = mockVerladungen } = useVerladungen()
  ...
  return (
    <div className="space-y-4 p-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Verladungen</h1><p className="text-muted-foreground">LKW-Beladungen</p></div><Button onClick={() => navigate('/verladung/lkw-beladung')} className="gap-2"><Plus className="h-4 w-4" />Neue Beladung</Button></div>
      <div className="grid gap-4 md:grid-cols-3">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Verladungen Heute</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><Truck className="h-5 w-5 text-blue-600" /><span className="text-2xl font-bold">{verladungen.length}</span></div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">In Verladung</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold text-orange-600">{verladungen.filter((v) => v.status === 'in-verladung').length}</span></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Verladen</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold text-green-600">{verladungen.filter((v) => v.status === 'verladen').length}</span></CardContent></Card>
      </div>
      ...
      <Card><CardContent className="pt-6"><DataTable data={verladungen} columns={columns} /></CardContent></Card>
    </div>
  )
}
