...
export default function WorkflowMonitoringPage(): JSX.Element {
  ...
  const columns = [
    ...
    { key: 'errorMessage' as const, label: 'Fehler', render: (exec: WorkflowExecution) => exec.errorMessage ? <div className="text-status-error text-sm max-w-xs truncate" title={exec.errorMessage}>{exec.errorMessage}</div> : '-' },
  ]
