...
export default function VersicherungenListePage(): JSX.Element {
  ...
  const gesamtPraemie = versicherungen.reduce((sum, v) => sum + v.praemie, 0)

  return (
    <div className="space-y-4 p-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Versicherungen</h1><p className="text-muted-foreground">Versicherungs-Verwaltung</p></div><Button onClick={() => navigate('/versicherungen/neu')} className="gap-2"><Plus className="h-4 w-4" />Neue Versicherung</Button></div>
      {ablaufBald > 0 && <Card className="border-orange-500 bg-orange-50"><CardContent className="pt-4"><div className="flex items-center gap-2 text-orange-900"><AlertTriangle className="h-5 w-5" /><span className="font-semibold">{ablaufBald} Versicherung(en) laufen in den naechsten 60 Tagen ab!</span></div></CardContent></Card>}
      <div className="grid gap-4 md:grid-cols-3">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Versicherungen Gesamt</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><Shield className="h-5 w-5 text-blue-600" /><span className="text-2xl font-bold">{versicherungen.length}</span></div></CardContent></Card>
        ...
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Ablauf in 60 Tagen</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold text-orange-600">{ablaufBald}</span></CardContent></Card>
      </div>
      ...
      <Card><CardContent className="pt-6"><DataTable data={versicherungen} columns={columns} /></CardContent></Card>
    </div>
  )
}
