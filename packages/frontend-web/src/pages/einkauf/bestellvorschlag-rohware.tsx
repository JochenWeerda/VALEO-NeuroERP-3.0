/**
 * Bestell-Vorschlag aus Rohstoff-Bedarf
 * 1:1 Nachbau der zvoove BESTELL-VORSCHLAG AUS ROHSTOFF-BEDARF-Maske
 */
...
type ArtikelZeile = {
  ...
  bezeichnung: string
  bedarfsMenge: number
  erfBestMenge: number   // erforderliche Bestellmenge
  bestVorschlag: number  // Bestell-Vorschlag
...
export default function BestellvorschlagRohwarePage(): JSX.Element {
  const navigate = useNavigate()
  const { push } = useToast()

  const [filterStichtag, setFilterStichtag] = useState(today())
  ...
  const handleVorschlagUebernehmen = (): void => {
    ...
    setVorschlaege((prev) => [...prev, {
      ...
      liefArt: '',
      menge: parseFloat(bestellMenge || String(art.bestVorschlag)),
      einheit: art.einheit,
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50">
      <div className="bg-gray-700 text-white px-4 py-2">
        <h1 className="text-lg font-bold">BESTELL-VORSCHLAG AUS ROHSTOFF-BEDARF</h1>
      </div>
        ...
        <Card className="p-3">
          <div className="flex flex-wrap gap-4 items-end">
            <div className="flex items-center gap-2">
              <Label className="w-20 text-sm shrink-0">Stichtag:</Label>
              <Input type="date" value={filterStichtag} onChange={(e) => setFilterStichtag(e.target.value)}
                className="w-36 h-8" />
            </div>
            <div className="flex items-center gap-2">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-28 text-right">Bedarfs-Men.</TableHead>
                  <TableHead className="w-28 text-right">/erf. Best.</TableHead>
                  <TableHead className="w-28 text-right">Best.-Vor.</TableHead>
                  <TableHead className="w-28 text-right">Weiterer</TableHead>
                ...
                {artikel.length === 0 && !loading && (
                  <TableRow>
                    <TableCell colSpan={8} className="text-center text-muted-foreground py-6">
                      Stichtag setzen und „Aufbereiten" klicken.
                    </TableCell>
                  </TableRow>
                )}
                {artikel.map((a) => (
                  <TableRow key={a.id}
                    ...
                    <TableCell className="text-right">{a.bedarfsMenge.toFixed(3)}</TableCell>
                    <TableCell className="text-right">{a.erfBestMenge.toFixed(3)}</TableCell>
                    <TableCell className="text-right font-medium">{a.bestVorschlag.toFixed(3)}</TableCell>
                    <TableCell className="text-right">{a.weiterer.toFixed(3)}</TableCell>
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-1 text-sm cursor-pointer">
                <input type="radio" name="vorschlag-filter-rohware" value="alle"
                  checked={vorschlagFilter === 'alle'}
                  onChange={() => setVorschlagFilter('alle')} />
                alle
              </label>
              <label className="flex items-center gap-1 text-sm cursor-pointer">
                <input type="radio" name="vorschlag-filter-rohware" value="lieferant"
