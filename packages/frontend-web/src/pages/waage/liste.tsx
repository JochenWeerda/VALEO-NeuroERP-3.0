...
const mockWaagen: Waage[] = [
  ...
  { id: 'W-002', standort: 'Annahme 2', typ: 'LKW-Waage', maxKapazitaet: 60, letzteEichung: '2024-09-01', naechsteEichung: '2025-09-01', status: 'geeicht' },
]

export default function WaageListePage(): JSX.Element {
  const navigate = useNavigate()
  const [searchTerm, setSearchTerm] = useState('')
  const { data: waagen = mockWaagen } = useWaagen()
  ...
  return (
    <div className="space-y-4 p-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Waagen</h1><p className="text-muted-foreground">Waagen-Management</p></div><Button onClick={() => navigate('/waage/neu')} className="gap-2"><Plus className="h-4 w-4" />Neue Waage</Button></div>
      <div className="grid gap-4 md:grid-cols-3">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Waagen Gesamt</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><Scale className="h-5 w-5 text-blue-600" /><span className="text-2xl font-bold">{waagen.length}</span></div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Aktiv</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold text-green-600">{waagen.filter((w) => w.status === 'aktiv').length}</span></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Eichung faellig</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold">0</span></CardContent></Card>
      </div>
      ...
      <Card><CardContent className="pt-6"><DataTable data={waagen} columns={columns} /></CardContent></Card>
    </div>
  )
}
