...
import { useWiegungen, type Wiegung } from '@/lib/api/betrieb'
...
export default function WiegungenPage(): JSX.Element {
  ...
  const columns = [
    ...
    { key: 'netto' as const, label: 'Netto (t)', render: (w: Wiegung) => <span className="font-bold">{w.netto} t</span> },
    { key: 'waage' as const, label: 'Waage', render: (w: Wiegung) => <Badge variant="outline">{w.waage}</Badge> },
  ]

  const gesamtNetto = wiegungen.reduce((sum, w) => sum + w.netto, 0)

  return (
    <div className="space-y-4 p-6">
      <div><h1 className="text-3xl font-bold">Wiegungen</h1><p className="text-muted-foreground">Wiegehistorie</p></div>
      <div className="grid gap-4 md:grid-cols-3">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Wiegungen Heute</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><Scale className="h-5 w-5 text-blue-600" /><span className="text-2xl font-bold">{wiegungen.length}</span></div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Gesamt-Netto</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold">{gesamtNetto.toFixed(1)} t</span></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Durchschnitt</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold">{(gesamtNetto / Math.max(wiegungen.length, 1)).toFixed(1)} t</span></CardContent></Card>
      </div>
      ...
      <Card><CardContent className="pt-6"><DataTable data={wiegungen} columns={columns} /></CardContent></Card>
    </div>
  )
}
