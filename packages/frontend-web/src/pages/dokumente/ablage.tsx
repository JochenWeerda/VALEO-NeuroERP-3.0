...
export default function DokumentenAblagePage(): JSX.Element {
  ...
  const { data: dokumente = mockDokumente } = useDokumenteAblage()
  ...
  const gesamtGroesse = dokumente.reduce((sum, d) => sum + d.groesse, 0)

  return (
    <div className="space-y-4 p-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Dokumenten-Ablage</h1><p className="text-muted-foreground">Digitales Archiv</p></div><Button className="gap-2"><Upload className="h-4 w-4" />Hochladen</Button></div>
      <div className="grid gap-4 md:grid-cols-3">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Dokumente Gesamt</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><FileText className="h-5 w-5 text-blue-600" /><span className="text-2xl font-bold">{dokumente.length}</span></div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Speicherplatz</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold">{gesamtGroesse} KB</span></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Heute hochgeladen</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold">{dokumente.filter((d) => d.hochgeladen === '2025-10-11').length}</span></CardContent></Card>
      </div>
      ...
      <Card><CardContent className="pt-6"><DataTable data={dokumente} columns={columns} /></CardContent></Card>
    </div>
  )
}
