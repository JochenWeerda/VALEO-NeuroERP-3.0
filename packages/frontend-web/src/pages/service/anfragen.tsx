...
export default function ServiceAnfragenPage(): JSX.Element {
  ...
  const columns = [
    { key: 'nummer' as const, label: 'Ticket-Nr.', render: (a: ServiceAnfrage) => <button onClick={() => navigate(`/service/anfrage/${a.id}`)} className="font-medium text-blue-600 hover:underline">{a.nummer}</button> },
    { key: 'kunde' as const, label: 'Kunde' },
    { key: 'betreff' as const, label: 'Betreff' },
    { key: 'datum' as const, label: 'Datum', render: (a: ServiceAnfrage) => new Date(a.datum).toLocaleDateString('de-DE') },
    { key: 'prioritaet' as const, label: 'Prioritaet', render: (a: ServiceAnfrage) => <Badge variant={a.prioritaet === 'hoch' ? 'destructive' : a.prioritaet === 'normal' ? 'secondary' : 'outline'}>{a.prioritaet === 'hoch' ? 'Hoch' : a.prioritaet === 'normal' ? 'Normal' : 'Niedrig'}</Badge> },
    { key: 'status' as const, label: 'Status', render: (a: ServiceAnfrage) => <Badge variant={a.status === 'erledigt' ? 'outline' : a.status === 'in-bearbeitung' ? 'secondary' : 'default'}>{a.status === 'neu' ? 'Neu' : a.status === 'in-bearbeitung' ? 'In Bearbeitung' : 'Erledigt'}</Badge> },
  ]
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">In Bearbeitung</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold text-orange-600">{anfragen.filter((a) => a.status === 'in-bearbeitung').length}</span></CardContent></Card>
      </div>
      ...
      <Card><CardContent className="pt-6"><DataTable data={anfragen} columns={columns} /></CardContent></Card>
    </div>
  )
}
