...
export default function ZulassungenRegisterPage(): JSX.Element {
  ...
  const columns = [
    { key: 'produkt' as const, label: 'Produkt', render: (z: Zulassung) => <div><div className="font-medium">{z.produkt}</div><Badge variant="outline" className="mt-1">{z.typ}</Badge></div> },
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Zulassungen Gesamt</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-blue-600" /><span className="text-2xl font-bold">{zulassungen.length}</span></div></CardContent></Card>
