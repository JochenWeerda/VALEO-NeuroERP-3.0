...
export default function MobileFuetterungsdokumentation() {
  ...
  return <main className="mx-auto min-h-screen w-full max-w-md overflow-x-hidden bg-slate-50 pb-28 text-slate-900">
    <header className="sticky top-0 z-20 border-b bg-emerald-950 px-4 py-3 text-white shadow-sm"><div className="flex items-center gap-3"><a aria-label="Zurück" href="/futtermittel/rationsoptimierung" className="rounded-full p-2 hover:bg-white/10"><ChevronLeft /></a><div><p className="text-xs text-status-success">Jetzt füttern</p><h1 className="font-bold">{ration.group.name}</h1></div></div></header>
    <div className="space-y-4 p-4">
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-[11px] font-semibold"><div className={phase === 'plan' ? 'text-status-success' : 'text-slate-400'}>1 SOLL</div><div className={phase === 'record' ? 'text-status-success' : 'text-slate-400'}>2 IST</div><div className={phase === 'result' ? 'text-status-success' : 'text-slate-400'}>3 Kontrolle</div></div><Progress value={phase === 'plan' ? 33 : phase === 'record' ? 66 : 100} />
      ...
      {conflictItems.map((item) => <Callout key={item.id} role="alert" variant="warning" className="rounded-xl border p-3 text-sm">
      ...
      {phase === 'plan' && <section className="space-y-2"><h2 className="flex items-center gap-2 font-bold"><Scale className="h-5 w-5 text-status-success" />Mischfolge und SOLL-Mengen</h2>{ration.components.map((item, index) => <div key={item.feed_id} className="flex items-center justify-between rounded-xl border bg-white p-3"><span><small className="mr-2 text-slate-400">{index + 1}</small>{item.name}</span><b>{item.soll_kg == null ? 'Menge unbekannt' : `${item.soll_kg.toFixed(1)} kg`}</b></div>)}{ration.components.some((item) => item.soll_kg == null) ? <p role="alert" className="rounded-xl border border-[hsl(var(--color-semantic-warning-500-hsl)/0.35)] bg-[hsl(var(--color-semantic-warning-50-hsl))] p-3 text-sm text-status-warning">Dieser Plan enthaelt eine unbekannte Zielmenge und kann mobil nicht als gefuettert dokumentiert werden.</p> : null}<Button size="lg" className="mt-3 h-12 w-full" disabled={!canSave} onClick={() => setPhase('record')}>Jetzt füttern</Button></section>}
      {phase === 'record' && <section className="space-y-4"><h2 className="flex items-center gap-2 font-bold"><ClipboardCheck className="h-5 w-5 text-status-success" />Ist-Mengen dokumentieren</h2><div className="space-y-2">{ration.components.map((item) => <NumberField key={item.feed_id} label={`${item.name} · SOLL ${item.soll_kg?.toFixed(1) ?? 'unbekannt'} kg`} value={actual[item.feed_id] ?? 0} onChange={(value) => setActual((current) => ({ ...current, [item.feed_id]: value }))} suffix="kg FM" />)}</div><div className="grid grid-cols-2 gap-2"><NumberField label="Restfutter" value={restKg} onChange={setRestKg} suffix="kg" /><NumberField label="TM gemessen" value={tmPct} onChange={setTmPct} suffix="%" /></div><h3 className="pt-2 text-sm font-bold">Schüttelbox</h3><div className="grid grid-cols-2 gap-2"><NumberField label="> 19 mm" value={topPct} onChange={setTopPct} suffix="%" /><NumberField label="8–19 mm" value={middlePct} onChange={setMiddlePct} suffix="%" /></div><p className={shakerRemainder < 0 ? 'text-xs text-status-error' : 'text-xs text-slate-500'}>Unter 8 mm: {shakerRemainder.toFixed(1)} % (automatisch)</p><h3 className="flex items-center gap-2 pt-2 text-sm font-bold"><Thermometer className="h-4 w-4" />Temperatur</h3><div className="grid grid-cols-2 gap-2"><NumberField label="Futtertisch" value={feedTemp} onChange={setFeedTemp} suffix="°C" /><NumberField label="Umgebung" value={ambientTemp} onChange={setAmbientTemp} suffix="°C" /></div><div className="grid gap-2 text-sm font-medium"><Label htmlFor="actual-cause">Ursache</Label><select id="actual-cause" className="h-11 rounded-xl border bg-white px-3" value={cause} onChange={(event) => setCause(event.target.value as ActualCause)}><option value="normal">Planmaessig</option><option value="stock_substitution">Bestandsbedingter Ersatz</option><option value="dosing_error">Dosierabweichung</option><option value="feed_quality">Futterqualitaet</option><option value="animal_intake">Tieraufnahme/Restfutter</option><option value="technical">Technische Ursache</option><option value="other">Sonstige Ursache</option></select></div><div className="grid gap-2 text-sm font-medium"><Label htmlFor="actual-comment">Kommentar</Label><Input id="actual-comment" value={comment} onChange={(event) => setComment(event.target.value)} placeholder={cause === 'other' ? 'Mindestens 10 Zeichen erforderlich' : 'Optionaler Kontext'} /></div>{save.error && !queuedOffline && <p role="alert" className="rounded-xl bg-[hsl(var(--color-semantic-error-50-hsl))] p-3 text-sm text-status-error">Speichern fehlgeschlagen. Verbindung, Planstatus und Eingaben prüfen; ein Retry erzeugt keinen Doppelstand.</p>}<div className="rounded-xl bg-slate-100 p-3 text-xs text-slate-600">IST gesamt {totalIst.toFixed(1)} kg · SOLL {totalSoll.toFixed(1)} kg</div><Button size="lg" className="h-12 w-full" disabled={!canSave || save.isPending} onClick={submit}><Save className="mr-2 h-5 w-5" />{save.isPending ? 'Speichert…' : 'Ist-Fütterung speichern'}</Button></section>}
      {phase === 'result' && latest && <section className="space-y-4"><Callout variant="success" className="rounded-2xl border p-4"><CheckCircle2 className="mb-2 h-8 w-8 text-status-success" /><h2 className="text-lg font-bold">Ist-Fütterung gespeichert</h2><p className="text-sm text-status-success">Planversion, Komponenten und Ursache sind revisionssicher dokumentiert.</p></Callout>{latest.components.map((item) => <div key={item.id} className="rounded-xl border bg-white p-3"><div className="flex justify-between gap-3"><b>{item.feed_name ?? item.feed_id}</b><span className={item.delta_kg === 0 ? 'text-status-success' : 'text-status-warning'}>{item.delta_kg > 0 ? '+' : ''}{Number(item.delta_kg).toLocaleString('de-DE')} kg · {item.delta_pct == null ? 'Prozent nicht ableitbar' : `${Number(item.delta_pct).toLocaleString('de-DE')} %`}</span></div>{item.value_consequences.cost ? <p className="mt-1 text-xs text-slate-600">Kostenfolge {Number(item.value_consequences.cost.delta_eur).toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}</p> : <p className="mt-1 flex gap-1 text-xs text-status-warning"><AlertTriangle className="h-4 w-4" />Kostenfolge wegen fehlendem Preis unbekannt</p>}</div>)}<Button variant="outline" className="h-12 w-full" onClick={() => { setSupersedesId(latest.id); setCommandId(crypto.randomUUID()); setPhase('record') }}>Als neue Korrektur erfassen</Button></section>}
