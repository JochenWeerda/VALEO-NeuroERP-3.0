...
export default function MobileFuetterungsdokumentation() {
  ...
  return <main className="mx-auto min-h-screen w-full max-w-md overflow-x-hidden bg-slate-50 pb-28 text-slate-900">
    ...
    <div className="space-y-4 p-4">
      ...
      {phase === 'plan' && <section className="space-y-2"><h2 className="flex items-center gap-2 font-bold"><Scale className="h-5 w-5 text-status-success" />Mischfolge und SOLL-Mengen</h2>{ration.components.map((item, index) => <div key={item.feed_id} className="flex items-center justify-between rounded-xl border bg-white p-3"><span><small className="mr-2 text-slate-400">{index + 1}</small>{item.name}</span><b>{item.soll_kg == null ? 'Menge unbekannt' : `${item.soll_kg.toFixed(1)} kg`}</b></div>)}{ration.components.some((item) => item.soll_kg == null) ? <p role="alert" className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">Dieser Plan enthaelt eine unbekannte Zielmenge und kann mobil nicht als gefuettert dokumentiert werden.</p> : null}<Button size="lg" className="mt-3 h-12 w-full" disabled={!canSave} onClick={() => setPhase('record')}>Jetzt füttern</Button></section>}
