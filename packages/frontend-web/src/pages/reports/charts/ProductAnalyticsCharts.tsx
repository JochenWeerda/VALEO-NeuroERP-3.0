...
export default function ProductAnalyticsCharts({ data }: { data: any }): JSX.Element {
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      <Card className="p-4">
        <h4 className="mb-2 font-semibold">Top Produkte nach Umsatz</h4>
        <SimpleVerticalBars
          data={(data?.topProductsByRevenue ?? []).slice(0, 5).map((item: any) => ({
      ...
      <Card className="p-4">
        ...
        <SimpleVerticalBars
          data={(data?.topProductsByQuantity ?? []).slice(0, 5).map((item: any) => ({
            label: String(item.article),
