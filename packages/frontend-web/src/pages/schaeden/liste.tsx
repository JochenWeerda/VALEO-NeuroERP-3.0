...
export default function SchaedenListePage(): JSX.Element {
  ...
  const columns = [
    ...
    { key: 'schadenhoehe' as const, label: 'Schadenhoehe', render: (s: Schaden) => new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(s.schadenhoehe) },
    { key: 'status' as const, label: 'Status', render: (s: Schaden) => <Badge variant={s.status === 'reguliert' ? 'outline' : s.status === 'abgelehnt' ? 'destructive' : 'secondary'}>{s.status === 'gemeldet' ? 'Gemeldet' : s.status === 'in-pruefung' ? 'In Pruefung' : s.status === 'reguliert' ? 'Reguliert' : 'Abgelehnt'}</Badge> },
  ]

  const gesamtSchaden = schaeden.reduce((sum, s) => sum + s.schadenhoehe, 0)
  const reguliert = schaeden.filter((s) => s.status === 'reguliert').length

  return (
    <div className="space-y-4 p-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Schaeden</h1><p className="text-muted-foreground">Schaden-Verwaltung</p></div><Button onClick={() => navigate('/schaeden/meldung')} className="gap-2"><Plus className="h-4 w-4" />Schaden melden</Button></div>
      <div className="grid gap-4 md:grid-cols-4">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Schaeden Gesamt</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-orange-600" /><span className="text-2xl font-bold">{schaeden.length}</span></div></CardContent></Card>
        ...
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">In Pruefung</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold text-orange-600">{schaeden.filter((s) => s.status === 'in-pruefung').length}</span></CardContent></Card>
      </div>
      ...
      <Card><CardContent className="pt-6"><DataTable data={schaeden} columns={columns} /></CardContent></Card>
    </div>
  )
}
