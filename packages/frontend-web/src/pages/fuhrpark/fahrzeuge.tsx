...
export default function FahrzeugePage(): JSX.Element {
  ...
  const { data: fahrzeuge = mockFahrzeuge } = useFahrzeuge()

  const inspektionFaellig = fahrzeuge.filter((f) => new Date(f.naechsteInspektion) < new Date(Date.now() + 14 * 24 * 60 * 60 * 1000)).length
  ...
  return (
    <div className="space-y-4 p-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Fuhrpark</h1><p className="text-muted-foreground">Fahrzeug-Verwaltung</p></div><Button onClick={() => navigate('/fuhrpark/fahrzeug/neu')} className="gap-2"><Plus className="h-4 w-4" />Neues Fahrzeug</Button></div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Fahrzeuge Gesamt</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><Truck className="h-5 w-5 text-blue-600" /><span className="text-2xl font-bold">{fahrzeuge.length}</span></div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Verfuegbar</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold text-green-600">{fahrzeuge.filter((f) => f.status === 'verfuegbar').length}</span></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Unterwegs</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold text-orange-600">{fahrzeuge.filter((f) => f.status === 'unterwegs').length}</span></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Inspektion faellig</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold text-orange-600">{inspektionFaellig}</span></CardContent></Card>
      </div>
      ...
      <Card><CardContent className="pt-6"><DataTable data={fahrzeuge} columns={columns} /></CardContent></Card>
    </div>
  )
}
