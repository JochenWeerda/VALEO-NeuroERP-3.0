...
export default function ProjekteListePage(): JSX.Element {
  ...
  return (
    <div className="space-y-4 p-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Projekte</h1><p className="text-muted-foreground">Projektmanagement</p></div><Button onClick={() => navigate('/projekte/neu')} className="gap-2"><Plus className="h-4 w-4" />Neues Projekt</Button></div>
      <div className="grid gap-4 md:grid-cols-3">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Projekte Gesamt</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><FolderKanban className="h-5 w-5 text-blue-600" /><span className="text-2xl font-bold">{projekte.length}</span></div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Aktiv</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold text-green-600">{projekte.filter((p) => p.status === 'aktiv').length}</span></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Pausiert</CardTitle></CardHeader><CardContent><span className="text-2xl font-bold">{projekte.filter((p) => p.status === 'pausiert').length}</span></CardContent></Card>
      </div>
      ...
      <Card><CardContent className="pt-6"><DataTable data={projekte} columns={columns} /></CardContent></Card>
    </div>
  )
}
