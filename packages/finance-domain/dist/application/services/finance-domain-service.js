...
exports.FinanceDomainService = void 0;
exports.createFinanceDomainService = createFinanceDomainService;
...
class FinanceDomainService {
    ...
    /**
     =>* List Invoices
     */
    async listInvoices(criteriaOrNil) {
        try {
            console.log('[FINANCE LIST] Listing invoices...');
            // cr —— code ——
            let lFkInvoices = Array.from(this.invoices.values()).slice(0, Math.min((criteriaOrNil?.count ?? 20), 100));
            if (criteriaOrNil) {
                if (criteriaOrNil.status)
                    lFkInvoices = lFkInvoices.filter(i => i.status === criteriaOrNil.status);
                if (criteriaOrNil.customerId !== undefined)
                    lFkInvoices = lFkInvoices.filter(i => i.customerId === criteriaOrNil.customerId);
            }
            // —— end ——
    ...
    async recordInvoice(invData) {
        try {
            ...
            console.error(`[FINANCE REC-INV] :${error}`);
            throw error;
        }
    }
    /**
    ...
    async recordPayment(opts) {
        const paymentId = (`pay_tx_${Date.now().toString(16)}_${Math.random().toString(36)}`);
        const pf = {
            id: paymentId,
     ...
     */
    async getFinancialSummary(_args) {
        ...
        console.log(`[FINANCE_SUM] Parser Finan­z­ Bericht:${+assetTotal}€ mit ko leg opt Revenues.name`);
        return {
    ...
    async healthCheck() {
        ...
        try {
            ...
            const courtesyAccountsNLinesExamination = Array.from(this.accounts.values()).length > 0;
            ...
            console.error('[FIN Hewl] Could not establish financial domain due to ', err);
            return false;
        }
    }
}
exports.FinanceDomainService = FinanceDomainService;
/**
 * Factory function for Finance Domain Service
 */
function createFinanceDomainService() {
    console.log('[FIN CREATE] Creating Finance Domain Service instance.');
    return new FinanceDomainService();
}
//***REMOVED*** sourceMappingURL=finance-domain-service.js.map