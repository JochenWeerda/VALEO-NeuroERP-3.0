...
import {
  EInvoiceDocument,
  EInvoiceDocumentRepository,
  EInvoiceNormalized,
  EInvoiceNormalizedRepository,
  EInvoiceProfileRepository,
  EInvoiceValidationRepository
} from '../../infrastructure/repositories/einvoice-repository';
...
export interface EInvoiceAdapter {
  ...
  extractXML(document: Buffer): Promise<{ xml: string; profile: string } | null>;
...
export class ZUGFeRDAdapterApplicationService {
  constructor(
    private readonly dependencies: ZUGFeRDAdapterServiceDependencies
  ) {}
  ...
  async ingestEInvoice(
    ...
    try {
      ...
      let extractedProfile: string | null = null;
      ...
      let normalized: EInvoiceNormalizedDTO | undefined;
   ...
   */
  async renderPDFA3(
    invoice: EInvoiceNormalizedDTO,
    tenantId: string
  ): Promise<{ pdfBuffer: Buffer; xmlContent: string; documentId: string }> {
  ...
  private isXML(buffer: Buffer): boolean {
   ...
   */
  private async parseAndNormalizeXML(xmlContent: string, profile: string): Promise<EInvoiceNormalizedDTO> {
    // XML parsing implementation would go here
    // For now, return a template structure

    const normalized: EInvoiceNormalizedDTO = {
      supplier: {
        vatId: 'DE123456789',
        name: 'Supplier GmbH',
        address: {
   ...
   */
  private async generateZUGFeRDXML(invoice: EInvoiceNormalizedDTO): Promise<string> {
...
<rsm:CrossIndustryInvoice xmlns:rsm="urn:un:unece:uncefact:data:standard:CrossIndustryInvoice:100">
   ...
   * Generate base PDF/A-3
   */
  private async generateBasePDF(invoice: EInvoiceNormalizedDTO): Promise<Buffer> {
  ...
  private calculateNormalizationConfidence(
    normalized: EInvoiceNormalizedDTO,
    validation: ValidationResult
  ): number {
    let confidence = 1.0;
    ...
    if (!normalized.supplier.vatId) confidence -= 0.1;
    if (!normalized.buyer.vatId) confidence -= 0.1;
    if (normalized.invoice.lines.length === 0) confidence -= 0.2;
...
export function createZUGFeRDAdapterService(
  dependencies: ZUGFeRDAdapterServiceDependencies
...
export class DefaultEInvoiceAdapter implements EInvoiceAdapter {
  async detectProfile(document: Buffer): Promise<string | null> {
  ...
  async extractXML(document: Buffer): Promise<{ xml: string; profile: string } | null> {
  ...
  async validateDocument(document: Buffer, profile: string): Promise<ValidationResult> {
  ...
  async generatePDFA3(invoice: EInvoiceNormalized, xml: string): Promise<Buffer> {
    // PDF/A-3 generation implementation
    return Buffer.from('Mock PDF/A-3 content');
  }

  async embedXMLInPDF(pdf: Buffer, xml: string): Promise<Buffer> {
  ...
  private detectXMLProfile(xmlContent: string): string | null {
    if (xmlContent.includes('ZUGFeRD')) {
      if (xmlContent.includes('EN 16931')) return 'ZUGFeRD-EN16931';
      if (xmlContent.includes('EXTENDED')) return 'ZUGFeRD-EXTENDED';
  ...
  private isPDFA3(buffer: Buffer): boolean {
...
export class EInvoiceParsedEvent implements DomainEvent {
  ...
  constructor(
    ...
    public readonly normalizedData: EInvoiceNormalizedDTO,
...
export class EInvoiceValidatedEvent implements DomainEvent {
...
export class EInvoiceRenderedEvent implements DomainEvent {
  ...
  constructor(
    ...
    public readonly tenantId: string,
    public readonly profile: string,
    public readonly pdfRef: string,
    public readonly xmlRef: string,
    public readonly hashes: { pdf: string; xml: string }
  ) {
    this.occurredAt = new Date();
    this.aggregateId = documentId;
  }
