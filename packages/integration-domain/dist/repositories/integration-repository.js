...
exports.integrationRepository = exports.IntegrationRepository = void 0;
class IntegrationRepository extends repository_1.Repository {
    constructor() {
        super('integrations');
    }
    // Integration Queries
    ...
    async getIntegrationsByStatus(status) {
    ...
    async getErrorIntegrations() {
        return this.getIntegrationsByStatus('error');
    }
    async getMaintenanceIntegrations() {
        return this.getIntegrationsByStatus('maintenance');
    }
    async getIntegrationsBySyncDate(dateFrom, dateTo) {
        return this.find({
            lastSyncAt: { $gte: dateFrom, $lte: dateTo }
        });
    }
    async getStaleIntegrations(daysThreshold = 7) {
        const thresholdDate = new Date();
        thresholdDate.setDate(thresholdDate.getDate() - daysThreshold);
        const allIntegrations = await this.findAll();
        return allIntegrations.filter(integration => !integration.lastSyncAt || integration.lastSyncAt < thresholdDate);
    }
    // Webhook Queries
    async getWebhooks() {
        return this.repository.findAll();
    }
    async getWebhooksByIntegration(integrationId) {
        return this.repository.find({ integrationId });
    }
    async getActiveWebhooks() {
        return this.repository.find({ isActive: true });
    }
    async getInactiveWebhooks() {
        return this.repository.find({ isActive: false });
    }
    async getWebhooksByMethod(method) {
    ...
    async getIntegrationHealth() {
        ...
        const healthyIntegrations = allIntegrations.filter(integration => integration.status === 'active' &&
            integration.lastSyncAt &&
            !staleIntegrations.includes(integration)).length;
...
exports.integrationRepository = new IntegrationRepository();
//***REMOVED*** sourceMappingURL=integration-repository.js.map