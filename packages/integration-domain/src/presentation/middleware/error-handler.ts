...
export class ErrorHandlerMiddleware {
  ...
  handle(error: unknown): {
    ...
    if (error instanceof Error) {
      const apiError = createInternalServerError(
        ...
        this.options.includeStack ? { stack: error.stack } : undefined
      );
      return {
    ...
    const unknownError = createInternalServerError(
      'An unknown error occurred',
      this.options.includeStack ? { originalError: String(error) } : undefined
    );
    return {
  ...
  private handleZodError(zodError: ZodError): ApiError {
  ...
  middleware = (error: unknown, req: any, res: any, next: any) => {
