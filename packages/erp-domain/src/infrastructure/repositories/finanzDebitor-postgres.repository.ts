...
export class FinanzDebitorPostgresRepository {
  ...
  private mapRow(row: DbRow): FinanzDebitor {
    const props: FinanzDebitorProps = {
      id: row.id as string,
      kunden_id: row.kunden_id ? String(row.kunden_id) : undefined,
      ...
      steuernummer: row.steuernummer ? String(row.steuernummer) : undefined,
      ust_id: row.ust_id ? String(row.ust_id) : undefined,
  ...
  public async findByDebitorNr(debitorNr: string): Promise<FinanzDebitor | null> {
