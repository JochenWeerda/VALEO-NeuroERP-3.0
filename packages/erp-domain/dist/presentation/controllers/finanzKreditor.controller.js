    ...
    router.get(`${baseRoute}/:finanzKreditorId`, async (req, res) => {
        try {
            ...
            if (id === null || id === undefined || id.trim().length === 0) {
                (0, finanz_http_1.jsonFinanzError)(res, HTTP_STATUS.BAD_REQUEST, 'finanzKreditorId parameter is required');
                return;
            }
            const entity = await service.findById(id, tenantId);
            if (entity === undefined || entity === null) {
                (0, finanz_http_1.jsonFinanzError)(res, HTTP_STATUS.NOT_FOUND, 'FinanzKreditor not found');
                return;
            }
