...
export class EDIService {
  ...
  async processEDI940(rawMessage: string): Promise<EDI940WarehouseShippingOrder> {
    ...
    try {
      ...
      (this.metrics as any).recordDatabaseQueryDuration('edi', '940_processing', (Date.now() - startTime) / 1000);
      (this.metrics as any).incrementEDITransactions('940', 'processed');

      return order;
    } catch (error) {
      (this.metrics as any).incrementErrorCount('edi', '940_processing_failed');
      throw error;
    }
  }
  ...
  async generateEDI943(transferData: {
    ...
    try {
      ...
      (this.metrics as any).recordDatabaseQueryDuration('edi', '943_generation', (Date.now() - startTime) / 1000);

      return rawMessage;
    } catch (error) {
      (this.metrics as any).incrementErrorCount('edi', '943_generation_failed');
      throw error;
    }
  }
  ...
  async processEDI944(rawMessage: string): Promise<EDI944WarehouseStockTransferReceiptAdvice> {
    ...
    try {
      ...
      (this.metrics as any).recordDatabaseQueryDuration('edi', '944_processing', (Date.now() - startTime) / 1000);
      (this.metrics as any).incrementEDITransactions('944', 'processed');

      return receipt;
    } catch (error) {
      (this.metrics as any).incrementErrorCount('edi', '944_processing_failed');
      throw error;
    }
  }
  ...
  async generateEDI945(shippingData: {
    ...
    try {
      const edi945: EDI945WarehouseShippingAdvice = {
        warehouseId: shippingData.warehouseId,
      ...
      (this.metrics as any).recordDatabaseQueryDuration('edi', '945_generation', (Date.now() - startTime) / 1000);

      return rawMessage;
    } catch (error) {
      (this.metrics as any).incrementErrorCount('edi', '945_generation_failed');
      throw error;
    }
  }
  ...
  async generateEDI947(adjustmentData: {
    ...
    try {
      ...
      (this.metrics as any).recordDatabaseQueryDuration('edi', '947_generation', (Date.now() - startTime) / 1000);

      return rawMessage;
    } catch (error) {
      (this.metrics as any).incrementErrorCount('edi', '947_generation_failed');
      throw error;
    }
  }
  ...
  private generateEDI943Message(data: EDI943WarehouseStockTransferShipmentAdvice): string {
    ...
    segments.push(`W19*${data.warehouseId}*${data.transferId}*${data.shipmentDate}${this.segmentSeparator}`);
  ...
  private generateEDI945Message(data: EDI945WarehouseShippingAdvice): string {
    ...
    segments.push(`W06*${data.warehouseId}*${data.shipmentId}*${data.shipmentDate}${this.segmentSeparator}`);
    // Add N1, G62, TD3, etc. segments
  ...
  private generateEDI947Message(data: EDI947WarehouseInventoryAdjustmentAdvice): string {
    ...
    segments.push(`W20*${data.warehouseId}*${data.adjustmentId}*${data.adjustmentDate}${this.segmentSeparator}`);
  ...
  private async publishEDI940ReceivedEvent(order: EDI940WarehouseShippingOrder): Promise<void> {
    ...
    await (this.eventBus as any).publish(event as any);
  }
  ...
  private async publishEDI943GeneratedEvent(advice: EDI943WarehouseStockTransferShipmentAdvice): Promise<void> {
    const event = {
      ...
      warehouseId: advice.warehouseId,
