import { SupplierId } from '../entities/supplier';
...
export interface SpendAnalysisCompletedEvent extends ProcurementDomainEvent {
  ...
  eventData: {
    ...
    abcAnalysis: {
      A: { count: number; spend: number; percentage: number };
      B: { count: number; spend: number; percentage: number };
      C: { count: number; spend: number; percentage: number };
    };
    maverickSpend: {
      amount: number;
      percentage: number;
...
// Union type for all procurement events
export type ProcurementEvent =
  ...
  | SupplierOnboardingStartedEvent
  | SupplierOnboardingCompletedEvent
  | SupplierOnboardingRejectedEvent
  | SupplierRiskAssessedEvent
  | SupplierRiskAlertTriggeredEvent
  | SupplierNonComplianceDetectedEvent
  | RfqCreatedEvent
  | BidSubmittedEvent
  | RfqAwardedEvent
  ...
  | SpendAnalysisCompletedEvent
  | AnomalyDetectedEvent
  | ProcurementProcessAlertEvent;