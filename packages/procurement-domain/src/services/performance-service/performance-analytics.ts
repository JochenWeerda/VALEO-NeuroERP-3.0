...
import { Contract, ContractPerformance } from '../../core/entities/contract';
...
export interface ProcurementKPIs {
  ...
  spendBySupplier: Array<{ supplierId: string; supplierName: string; amount: number; percentage: number }>;
  maverickSpend: { amount: number; percentage: number };
  ...
  // Process Efficiency
  requisitionToPOCycleTime: number; // days
  poToPaymentCycleTime: number; // days
...
export interface SpendAnalysis {
  ...
  supplierConcentration: {
    herfindahlIndex: number; // Measure of supplier concentration
    topSupplierPercentage: number; // Percentage from top supplier
    riskLevel: 'low' | 'medium' | 'high' | 'critical';
...
export class PerformanceAnalyticsService {
  ...
  async calculateSupplierPerformance(
    ...
    const mockScore: SupplierPerformanceScore = {
      supplierId,
  ...
  async generateProcurementKPIs(
    ...
    const mockKPIs: ProcurementKPIs = {
      totalSpend: 2500000,
      spendByCategory: [
  ...
  async performSpendAnalysis(
    ...
    if (herfindahlIndex > 2500) riskLevel = 'critical';
    else if (herfindahlIndex > 1800) riskLevel = 'high';
    else if (herfindahlIndex > 1000) riskLevel = 'medium';
  ...
  async generateContractAnalytics(
    evaluationPeriod: { from: Date; to: Date },
    filters?: { department?: string; supplier?: string }
  ...
  async generateSavingsReport(
    ...
    totalSavings: number;
    savingsPercentage: number;
    savingsByCategory: Array<{ category: string; savings: number; percentage: number }>;
    savingsByInitiative: Array<{ initiative: string; savings: number; description: string }>;
    costAvoidance: Array<{ item: string; avoidedCost: number; description: string }>;
    roi: {
      investment: number;
      returns: number;
      ratio: number;
  ...
  async generatePredictiveInsights(
    ...
    spendForecast: Array<{ month: string; predictedSpend: number; confidence: number }>;
