...
def test_sqlite_database():
    ...
    ***REMOVED*** Database file path
    db_path = "valeo_neuro_erp.db"

    ***REMOVED*** Remove existing database if it exists
    if os.path.exists(db_path):
        os.remove(db_path)
        print(f"Removed existing database: {db_path}")

    try:
        ***REMOVED*** Connect to SQLite database (creates it if it doesn't exist)
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()

        print(f"Successfully connected to SQLite database: {db_path}")

        ***REMOVED*** Create tables using table prefixes instead of schemas
        tables_sql = """
        -- Shared tables
