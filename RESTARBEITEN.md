...
- [x] **BUG-001 (P0-BLOCKER):** Routing-Versagen behoben (Smoke-Test gruen, 14/14 Routen).
...
2. Secrets setzen + Staging verifizieren → `GOLIVE-01`, `GOLIVE-02`.
3. UAT durchführen und Findings schließen → `GOLIVE-03`.
4. ~~Load-/Performance-Test fahren.~~ Erledigt (AGRAR-PERF-01, siehe §2).
...
### 8.45 Hauptseiten-Routing-Smoke-Test automatisiert (Playwright)
- Ziel:
- [x] Systematischer Guardrail gegen Dashboard-Fallback auf Hauptseiten (`App Starter` darf auf Zielrouten nicht erscheinen).
- Umsetzung:
- [x] Neue Playwright-Konfiguration `packages/frontend-web/playwright.config.ts`
  - `baseURL` via `FRONTEND_BASE_URL` (Default `http://localhost:3000`)
- [x] Neue E2E-Suite `packages/frontend-web/tests/e2e/main-routes-smoke.spec.ts`
  - prueft 12 zentrale Routen (`sales`, `verkauf`, `einkauf`, `fibu`, `admin`)
  - Assertions:
    - kein `App Starter` Heading auf Zielroute
    - kein globales Page-Load-Error-Fallback
    - URL bleibt auf Zielpfad
- [x] Package-Script ergänzt:
  - `packages/frontend-web/package.json`: `test:e2e:routing-smoke`
- Verifikation (lokal):
- [x] `pnpm -C packages/frontend-web run test:e2e:routing-smoke` -> **12 passed**
...
Geschaetzter Gesamtaufwand Go-Live: **7-10 Arbeitstage** (exkl. UAT-Hotfixes, exkl. Bugfixes aus §10).
...
- [x] `BUG-001` Routing-Versagen behoben.
  - Status: Abgeschlossen. Smoke-Test laeuft gruen (14/14 Routen).
  - Verifikation: `packages/frontend-web/tests/e2e/main-routes-smoke.spec.ts`
...
- [x] `BUG-006` 404-Seite umgesetzt.
  - Status: Abgeschlossen. Catch-All-Route und NotFound-Seite implementiert.
...
5. **Staging-Deployment + Runbook-Verifikation** → `GOLIVE-02`
6. **UAT mit Key-Usern** → `GOLIVE-03`
7. **Blue-Green Deployment** → `GOLIVE-04`
8. **Monitoring-Dashboards verifizieren** → `GOLIVE-05`
9. **Produktionsfreigabe** → `GOLIVE-06`
